...
## Five Levels, [source redacted]-Style
...
- Each level **explicitly defines what the character can now do**, building on the previous level's capability — this mirrors how [source redacted] work (e.g. [source redacted], Potence), where each dot unlocks a specific, named power rather than just a bigger number.
...
See [premade-gifts.md](premade-gifts.md) for a complete catalog of **55 Gifts, every one with all 5 levels fully defined**, organized into 15 alphabetized categories. Ready to hand a player at the table as-is. (Cybernetics' sole entry, *Grafted Steel*, was removed 2026-07-15 — see [premade-gifts.md](premade-gifts.md#cybernetics--note). *Bulwark*, granting Soak, was added the same day — see [premade-gifts.md](premade-gifts.md#bulwark-damage-resistance--toughness). Catalog count dropped from 77 to 61 across two consolidation passes on 2026-07-16, rose to 62 the same day with the addition of *Healer's Touch* (heal an ally by touch), dropped to 57 when Ember Touch, Frostbound, Stoneheart, Storm's Herald, Tidecaller, and Voltaic Pulse were consolidated into one *Elemental Manipulation* Gift, to 56 when Fortune's Favor and Ill Omen merged into *Fortune's Hand*, then to 55 when Radiant Face and Mind's Whisper merged into *Truthsense* — all during the monitored review pass, see [gift-review-progress.md](gift-review-progress.md) for the full log.)

## Open Questions

- **Exact Resonance cost to activate a Gift** — in active playtesting as of 2026-07-15, not yet locked to one formula. See [core-mechanic.md](core-mechanic.md#gift-activation-cost--in-playtesting).

## Deferred — Future Expansion

- **Cybernetics.** Currently has **zero catalog representation** — the sole entry, *Grafted Steel*, was removed 2026-07-15 (see [premade-gifts.md](premade-gifts.md#cybernetics--note)). Marked 2026-07-15 as a deliberate future-expansion item rather than a near-term open question: when it's picked back up, it needs more concepts (sensory augments, subdermal armor, combat-adjacent implants, neural interfaces, etc.) and a decision on whether cybernetics should follow the standard Gift creation process as-is, or needs its own acquisition rules (surgery, monetary cost, installation time/risk, rejection/compatibility) given it's typically bought and installed rather than trained or awakened into.
...
- **XP costs during Advancement, confirmed 2026-07-14**: acquiring a **brand-new Gift** (0→level 1) costs a **flat 7 XP**. Raising an **existing Gift's level** costs `current level × 5` — a direct import of [source redacted]'s in-clan Discipline cost. Climbing a Gift from level 1 to 5 costs `5 × (1+2+3+4)` = **50 XP** on top of the initial 7, for **57 XP total** to fully max one Gift from scratch.
- **Detail pass complete, and Resisted-Roll-vs-Resolve triage resolved, both 2026-07-15**: every Gift level across all 76 entries in [premade-gifts.md](premade-gifts.md) now specifies Action cost, Range, Duration, and Resolution, on top of richer flavor text for the Capability itself. Resolution was decided per-Gift-level as part of the same pass rather than as a separate future TODO: most Gifts use the Resolve default (Power Level + Attribute vs. the target's Resolve), and a Gift level is only flagged as a full Resisted Roll where the effect is a genuine two-sided contest (e.g. a strength contest in Overwhelming Might, or actively wrestling free of an intruding mental effect in Iron Mind/Untouchable). Gift Resonance activation cost is deliberately left as a formula reference rather than a fixed number in every entry, since it's still in playtesting — see [core-mechanic.md](core-mechanic.md#gift-activation-cost--in-playtesting).
- **New Gift creation happens only between sessions, confirmed 2026-07-15**: same timing as new Skill creation — subject to GM discussion and approval (scope, all 5 levels, thematic fit), not something a player invents on the fly mid-scene or slots in during character creation itself. Picking an *existing* catalog Gift at character creation (the normal Gift-pool spend) is unaffected — this only governs inventing a brand-new one from scratch.
- **Gifts require no in-fiction source or origin, confirmed 2026-07-15**: unlike a Perk (which grants its own standing without needing a prior justification either, by symmetry) or a [source redacted]-style bloodline/pact/awakening, a Gift here doesn't need a bloodline, pact, training regimen, or mutation to explain it — a character simply has it. Consistent with the setting's own "the world is weird" framing: exceptional ability doesn't need a tidy origin story to be real.
