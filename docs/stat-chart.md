...
|---|---|---|---|---|---|
| **Earth** | Physical power/endurance | **Soak** | Passive wall — each incoming damage die compared individually; die ≤ Soak is fully absorbed, die > Soak connects. Soak 10 = guaranteed 0% connect (true full negation, unless the attacker spends Fate Tokens to boost individual dice). | **Healing** | Undefined |
| **Air** | Agility/adaptability | **Initiative** | Rolled once at the start of combat (1d10 + Initiative), fixed for the whole fight. Higher acts first. | **Luck** | Undefined |
| **Fire** | Drive/aggression | **Damage** | Sets how many d10 are rolled on a hit — one die per point. Each die compared individually against the defender's Soak. | **Presence** | Undefined (in progress) |
...
| **Wyrd** | Fate/destiny/the supernatural | **Warp** | Derived stat: `Defense = 10 − Warp`. Defense becomes the attacker's Difficulty when targeting this character. | **Weft** | Undefined (candidate: refills Ki) |
...
Healing, Luck, Presence (in progress), Weft, Stamina's non-combat mechanic, what happens at 0 Health Levels, Health Level recovery, and the rest of the open-questions TODO list in [design-log.md](design-log.md#open-questions).
