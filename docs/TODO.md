...
**Soak resolved via a new Gift, Bulwark** (76th Gift) — flat Soak +1/Level, [source redacted] convention (fixed mechanic, any fictional source: tough skin, mystic ward, subdermal plating, etc.). No equipment subsystem needed. See [combat.md](../rules/combat.md#soak--confirmed).
...
- **Soak** (a flat integer, reduces damage after a hit connects) resolved 2026-07-15 — not gear/Attribute-derived after all, but granted by the new **Bulwark** Gift (+1 Soak/Level, [source redacted] convention: player picks the fictional source — tough skin, mystic ward, subdermal plating — same fixed numbers regardless). See [combat.md](../rules/combat.md#soak--confirmed).
...
- [x] **Full trigger list resolved as deliberately non-exhaustive** (2026-07-14): compiled an illustrative candidate list from [source redacted] (flanking, higher ground, surprise, teamwork, right tool/environment, narrative stunts, Passion-driven moments, Leadership-style rallying) — GM can recognize others as they arise. See [core-mechanic.md](../rules/core-mechanic.md#advantage-trigger-examples--non-exhaustive).
...
- [x] **Calibration step 1 done**: derived a comparable raw target number for "average human, average task" by anchoring to [source redacted]'s shared "10 = human average" convention — three reference conversions land in a 10-13 range, recommending **10** as the working baseline (see [character-creation/overview.md](../character-creation/overview.md#calibration)). This is an input to pool sizing, not the pool size itself.
...
- [x] **No separate starting-creation cap on Attributes** (2026-07-14): unlike [source redacted]'s "max 3 at creation," this system only bounds Attributes by their normal min 1/max 10 range plus what the 10-point pool can afford — no redundant lower ceiling on top of that.
...
- [x] **Gift pool size resolved: 8 points** (2026-07-14): checked against [source redacted]'s Sphere-dot budget (5, and the acknowledged direct inspiration for the Power Level/Gift Level split), Vampire's Discipline budget (3), and [source redacted]/Realms budget (~5) — 8 is ~1.6× Mage's baseline, consistent with how generous the Attribute/Skill pools already are relative to [source redacted]. Confirms the pool supports both specialization (one maxed Gift at 5 + a secondary at 3) and generalization (up to 4 Gifts at Level 2, or thinner spreads), assuming flat per-level cost. See [character-creation/overview.md](../character-creation/overview.md#calibration).
...
- [x] **Wildcard Points fully resolved** (2026-07-14, renamed from "Freebie Points" 2026-07-15): starting pool of **15** (flat, regardless of Flaws), spendable across all four categories at **Attribute 7/level (cap 2 levels), Skill 1/level (no extra cap), Perk 3/Perk (no cap), Gift 5/level (cap 3 levels)**. Flaws add to the 15 baseline: **1-7 Wildcard Points** per non-Leveled Flaw ([source redacted]'s severity range, negotiated), or **2 × level** for the 3 Leveled (Resource) Flaws, **capped at 15 total from Flaws** (resolved 2026-07-15). The 15 baseline alone can hit either the Attribute cap (14) or the Gift cap (15) but not both. See [character-creation/overview.md](../character-creation/overview.md#calibration).
...
**XP spend costs, confirmed 2026-07-14** — a [source redacted]-style `current rating × multiplier` scaling model, ratios deliberately weighted by how much systemic reach each category has:
...
- [x] **XP award rate resolved: 5 per session** (2026-07-14), via a [source redacted]-blended category checklist (Attendance, Roleplaying, Heroism, Learning, Standout Moment — 1 XP each) plus separate [source redacted]-style Story Awards for major milestones. Derived from the fact that most of our per-category costs are direct [source redacted] imports except Attribute (×9 vs. [source redacted]'s ×4) — reusing [source redacted]'s own 5 XP/session rate means everything [source redacted] advances at [source redacted]'s pace, and Attributes land 2.25× slower automatically. See [character-creation/overview.md](../character-creation/overview.md#advancement).
...
- [x] **Flaw list compiled** (resolved 2026-07-14): sourced from [source redacted] (numeric-penalty entries excluded, mirroring Perks' exclusion of numeric-bonus Feats). Categories: Physical, Mental/Cognitive, Social, Leveled (Resource), Combat-Adjacent. See [flaws.md](../rules/flaws.md#flaw-list).
...
- [x] **Bulwark added, 76th Gift** (2026-07-15) — grants Soak (flat damage reduction, +1/Level), [source redacted] convention (fixed mechanic, player-chosen fictional source). Resolves the long-standing Soak open item without needing an equipment subsystem. See [combat.md](../rules/combat.md#soak--confirmed) and [premade-gifts.md](../rules/premade-gifts.md#bulwark-damage-resistance).
