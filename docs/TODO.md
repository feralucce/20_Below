...
- [x] **Special Maneuvers fully mechanized** (now 10, from [source redacted]): Grapple/Trip/Escape (Resisted Rolls, status effects, no damage); Disarm/Called Shot (attack at −5 target penalty); All-Out Attack/Haymaker (Dodge-style DoS ×1.5/×2, defenseless 1 round vs. until next turn); Charge/Brace/Help (grant Advantage). Disengage removed.
- [x] Called shot difficulty modifier: locked at **−5**.
- [x] Called Shot location-effects proposal drafted ([source redacted] hit-location-inspired): Head (Stunned, TBD), Eyes (Blinded), Arms/Hands (Disarmed/Impaired), Legs/Feet (Move reduced), Torso/Vitals (reserved for later). Not yet confirmed.
