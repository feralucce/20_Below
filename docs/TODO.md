...
- [x] **Special Maneuvers fully mechanized** (11, from [source redacted]): Grapple/Trip/Escape (Resisted Rolls, status effects, no damage); Disarm/Called Shot (attack at −5 target penalty); All-Out Attack/Haymaker (Dodge-style DoS ×1.5/×2, defenseless 1 round vs. until next turn); Charge/Brace/Help (grant Advantage); Disengage (prevents a Reaction-trigger response).
