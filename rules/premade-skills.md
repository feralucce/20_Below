...
A starting Skill list scoped to a **modern-world base campaign**. Per [skills.md](skills.md), **this list is not exhaustive** - players and GMs should freely add, split, merge, or rename entries to fit a specific character or campaign. It exists to save table time at character creation, not to fence in what a character can plausibly do.
...
**Every Skill below has a Default Element** - the Attribute a roll uses unless the player [challenges it with a Descriptor](skills.md#skills-default-to-an-element). The default reflects the most natural, common way that Skill gets used; it isn't a hard rule about what the Skill *is*, just the assumption a roll starts from.
