...
**Soak** (a flat integer, applied after a hit connects via Defense) is not Attribute-derived — it comes from the [Bulwark](gifts.md) Gift (+1 per Level, [source redacted] convention: any fictional source, same fixed numbers), not a dedicated equipment subsystem. See [combat.md](combat.md#soak--confirmed) for the full mechanic.
...
**XP cost to raise Resonance during Advancement**: `current rating × 6` (**revised 2026-07-15**, down from ×7, to account for the wider 1-20 ceiling — climbing the same number of ranks now costs less per rank than Power Level's still-×7 rate, so a longer climb doesn't compound as steeply). Originally set at ×7 to match Power Level exactly (2026-07-14): priced as a "meta" stat, not [source redacted]'s own cheap Willpower rate (×1), since it gates **all** Gift activation and backs a 10-effect toolkit reaching well beyond Gifts. Still doesn't reach Attribute's ×9 because raising Resonance only grows *pool capacity* — every individual use still costs its own 1-3 points, and the pool refills slowly, so the always-on-bonus logic that justifies Attribute's premium doesn't fully apply here.
...
**Not yet locked to a single formula.** 2026-07-15 research pass compared several systems: [source redacted]/Gift/Art costs are largely **flat** per activation regardless of dot level (dot rating gates scope, not cost — the closest structural match to how this game's own Gift Level already works); [source redacted] (END = Active Points ÷ 10) and [source redacted] (Drain = Force ÷ 2, floored) both scale cost directly with power level, but both are point-buy/energy-invested models rather than [source redacted]-style discrete scope-gates. Rather than pick one on paper, **all three candidates go to the table**, trialed in order until one feels right:
...
3. **Flat 1, regardless of Level** — try third. Matches the [source redacted] precedent most directly; Gift Level does all the differentiating work, cost does none.
...
- **Gifts** — abilities that make a character truly exceptional. Scored **1-5**, not 1-10 — each Gift is unique/narrow in scope, and each of its 5 levels is a specific, cumulative, defined capability ([source redacted]). See [gifts.md](gifts.md).
...
- **Narrative / Roleplay-Driven**
  - A vividly or cleverly described action ("stunting," [source redacted]-style) — rewards descriptive engagement, distinct from the GM's general "genuinely uncertain" call.
  - A character's Passion, Flaw, or defining motivation directly driving the moment ([source redacted]-style).
- **Morale / Inspiration**
...
- **Outnumbered / Overwhelmed**
  - Being mobbed or surrounded by multiple opponents at once ([source redacted]).
...
- **Narrative / Roleplay-Driven**
  - Acting directly against a character's own Passion, Flaw, or defining motivation ([source redacted]-style — the inverse of a Passion driving the moment for Advantage).
