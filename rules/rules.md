...
This is a **direct Ki spend**, the same category as [spending Ki to preserve a Health Level](#ki-spend-to-preserve-a-health-level) and [Bump Action Bracket](fate.md#ki-the-pool-formerly-risk-pool) - **not** a Fate Token spend, so it does **not** trigger the [Fatigue Check](fate.md#fatigue-check) and does **not** count against [Stamina's per-Scene spend cap](fate.md#staminas-job).
