# The Adversary Index

A catalog of threats a party might face outside of other player characters - not a pure bestiary, since it will eventually include dangerous people alongside dangerous animals. Each entry gives a GM everything needed to run the creature at the table without a separate build process.

**How to read a stat block:**

- **Soak** and **Defense** work exactly as they do for a PC - Soak resolves per-die against an attack (rules.md#physical-attacks--weapon-damage--per-die-resolution), Defense is the target number for an attacker's to-hit roll.
- **Attack** is the creature's own Attribute rating (1-10) for making that to-hit roll - roll 2d10 against Attack + the target's Defense (used as Difficulty, per rules.md#the-passive-wall-triad---soak-presence-psyche), standard critical results apply.
- **Health Levels** work exactly as they do for a PC, with no below-0 range - a creature is out of the fight the instant it crosses 0.
- **Movement Rate** is in meters, same as a PC's.
- Each **Natural Weapon** lists a Damage rating (d10 count, same 1-10 scale as [weapons.md](weapons.md)) and a range.
- **Notable Skills** give a single number - treat it as the creature's full roll target whenever that Skill matters. Roll 2d10 against it, standard critical results apply, exactly like any other roll.
- **Traits** describe anything that doesn't fit the fields above, written in terms of mechanics that already exist (Advantage/Disadvantage, Range Bands, Ki, etc.) rather than new ones.
- **A small number of entries have an ability marked "no to-hit roll required."** Every attack normally starts with a to-hit roll against the target's Defense (rules.md#the-passive-wall-triad---soak-presence-psyche) - but a small handful of entries (things that affect you by being perceived rather than by striking you, like a banshee-style wail) don't fit that shape. For those specific abilities only, perceiving the source (a Perception check, or simply being in range and aware of it, GM's call) triggers the effect instead of a to-hit roll. Everything downstream of that - the dice resolving per-die against the relevant wall stat, Ki Spend to Preserve options for the target - works exactly like any other attack. Nothing else about the resolution changes.

---

## Mundane Beasts

Ordinary animals, native to a modern-day setting. None of these are supernatural - a description matching what a real specimen is actually capable of, run through the system's own math.

**Domestic & Urban**: [Feral Dog](#feral-dog) · [Guard Dog](#guard-dog) · [Feral Cat](#feral-cat) · [Raccoon](#raccoon) · [Skunk](#skunk) · [Rat Swarm](#rat-swarm)

**Farm & Rural**: [Bull](#bull) · [Wild Boar](#wild-boar) · [Aggressive Goose](#aggressive-goose) · [Spooked Horse](#spooked-horse) · [Feral Goat](#feral-goat)

**Canines**: [Coyote](#coyote) · [Wolf](#wolf) · [Wolverine](#wolverine)

**Wild Cats**: [Mountain Lion](#mountain-lion) · [Bobcat](#bobcat) · [Lynx](#lynx)

**Bears**: [Black Bear](#black-bear) · [Grizzly Bear](#grizzly-bear) · [Polar Bear](#polar-bear)

**Reptiles**: [Rattlesnake](#rattlesnake) · [Copperhead](#copperhead) · [Cottonmouth](#cottonmouth) · [Alligator](#alligator) · [Snapping Turtle](#snapping-turtle) · [Gila Monster](#gila-monster)

**Arachnids & Insects**: [Black Widow](#black-widow) · [Brown Recluse](#brown-recluse) · [Wasp Swarm](#wasp-swarm) · [Africanized Bee Swarm](#africanized-bee-swarm) · [Scorpion](#scorpion)

**Birds of Prey & Scavengers**: [Red-Tailed Hawk](#red-tailed-hawk) · [Great Horned Owl](#great-horned-owl) · [Turkey Vulture](#turkey-vulture)

**Aquatic**: [Great White Shark](#great-white-shark) · [Bull Shark](#bull-shark) · [Piranha School](#piranha-school) · [Orca](#orca) · [Electric Eel](#electric-eel)

**Large Herbivores**: [Bison](#bison) · [Moose](#moose) · [Elk](#elk)

**Arctic Specialists**: [Arctic Fox](#arctic-fox) · [Snowy Owl](#snowy-owl)

---

<table class="adversary-card"><tr><td markdown="1">

### Feral Dog

A stray or pack-abandoned dog, unpredictable more than dangerous - but a hungry or cornered one will bite.

**Soak** 1 · **Attack** 3 · **Defense** 6 · **Health Levels** 3 · **Movement Rate** 9m

**Bite**: 2, Melee

**Notable Skills**: Perception 8, Athletics 8

**Traits**: **Pack Tactics** - gains Advantage on an attack roll when at least one other canine is also attacking the same target.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Guard Dog

Bred or trained to hold a line, not just to threaten one.

**Soak** 1 · **Attack** 4 · **Defense** 6 · **Health Levels** 4 · **Movement Rate** 9m

**Bite**: 3, Melee

**Notable Skills**: Intimidation 8, Perception 9

**Traits**: **Trained to Hold** - immune to being Surprised by a target it's already been set to guard against, and doesn't break off an attack from fear alone.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Feral Cat

Small, fast, and almost never worth the fight it can put up if actually cornered.

**Soak** 0 · **Attack** 2 · **Defense** 8 · **Health Levels** 2 · **Movement Rate** 8m

**Claws**: 1, Melee

**Notable Skills**: Stealth 10, Athletics 9

**Traits**: **Hard to Corner** - climbs and leaps at no penalty; treat any vertical surface or gap within its Movement Rate as normal ground for the purpose of escaping.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Raccoon

Bold around people, genuinely dangerous only if rabid.

**Soak** 1 · **Attack** 3 · **Defense** 7 · **Health Levels** 2 · **Movement Rate** 6m

**Bite / Claws**: 1, Melee

**Notable Skills**: Stealth 8, Perception 7

**Traits**: **Rarely Aggressive** - won't initiate an attack unless cornered, defending young, or rabid. A rabid raccoon gains Advantage on its bite.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Skunk

No interest in a fight - just in being left alone.

**Soak** 0 · **Attack** 1 · **Defense** 6 · **Health Levels** 1 · **Movement Rate** 4m

**Spray** (once per encounter, no Health Level cost): every target within Close range must wash off or take Disadvantage on all rolls for the rest of the Scene.

**Notable Skills**: Perception 6

**Traits**: **Warning Display** - always gets one clearly-telegraphed round of stomping/hissing before spraying, unless directly attacked first.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Rat Swarm

A mass of small bodies, not a single creature - what it lacks in individual danger it makes up in sheer numbers.

**Soak** 0 · **Attack** 3 · **Defense** 8 · **Health Levels** 2 · **Movement Rate** 6m

**Bite**: 1, Melee

**Notable Skills**: Stealth 8

**Traits**: **Swarm** - immune to single-target Physical attacks (they connect with mostly empty space); any attack that hits an area instead deals double dice against it.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Bull

Placid until it isn't - and a full charge is genuinely lethal.

**Soak** 3 · **Attack** 6 · **Defense** 5 · **Health Levels** 6 · **Movement Rate** 10m

**Gore**: 5, Melee

**Notable Skills**: Athletics 9

**Traits**: **Charge** - if it moves its full Movement Rate directly at a target before attacking, its Gore rolls one additional die that round.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Wild Boar

Low, fast, and willing to press an attack that a smarter animal would break off.

**Soak** 2 · **Attack** 5 · **Defense** 5 · **Health Levels** 4 · **Movement Rate** 8m

**Tusks**: 3, Melee

**Notable Skills**: Athletics 8

**Traits**: **Low Profile** - Advantage on any roll to resist being tripped or knocked down.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Aggressive Goose

Absurd on paper, genuinely relentless in practice.

**Soak** 0 · **Attack** 2 · **Defense** 7 · **Health Levels** 1 · **Movement Rate** 6m

**Bite / Wing Buffet**: 1, Melee

**Notable Skills**: Intimidation 7

**Traits**: **Relentless** - keeps attacking after taking damage with no hesitation; only stops if it actually runs out of Health Levels or the target leaves its territory entirely.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Spooked Horse

Not a predator - a large, panicked animal is dangerous mostly by accident.

**Soak** 1 · **Attack** 4 · **Defense** 5 · **Health Levels** 5 · **Movement Rate** 14m

**Kick / Trample**: 4, Melee

**Notable Skills**: Athletics 9

**Traits**: **Trample** - if its movement this round passes through a Close-range space a target occupies, it can make its attack as part of that movement instead of spending a separate action.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Feral Goat

Territorial and stubborn, more nuisance than threat on its own.

**Soak** 1 · **Attack** 3 · **Defense** 6 · **Health Levels** 3 · **Movement Rate** 8m

**Headbutt**: 2, Melee

**Notable Skills**: Athletics 8

**Traits**: **Sure-Footed** - Advantage on any roll to keep its footing on unstable or steep terrain.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Coyote

Cautious and opportunistic - rarely presses a fight it isn't sure it can win.

**Soak** 1 · **Attack** 3 · **Defense** 7 · **Health Levels** 3 · **Movement Rate** 10m

**Bite**: 2, Melee

**Notable Skills**: Stealth 8, Perception 8

**Traits**: **Pack Tactics** (as Feral Dog).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Wolf

A genuine apex predator in its own territory, especially in numbers.

**Soak** 2 · **Attack** 5 · **Defense** 6 · **Health Levels** 4 · **Movement Rate** 11m

**Bite**: 4, Melee

**Notable Skills**: Perception 8, Athletics 8

**Traits**: **Pack Tactics** (as Feral Dog) - a wolf attacking a target already engaged by another wolf this Scene gains Advantage even without a second wolf attacking the same round.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Wolverine

Small for what it's willing to fight, and it doesn't quit.

**Soak** 3 · **Attack** 6 · **Defense** 5 · **Health Levels** 5 · **Movement Rate** 8m

**Bite / Claws**: 4, Melee

**Notable Skills**: Athletics 8

**Traits**: **Ferocious** - never the one to break off an engagement voluntarily; fights until it or its target is out of Health Levels or the target escapes outright.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mountain Lion

Rarely seen before it attacks - that's the point.

**Soak** 2 · **Attack** 6 · **Defense** 6 · **Health Levels** 5 · **Movement Rate** 12m

**Claws / Bite**: 5, Melee

**Notable Skills**: Stealth 10, Athletics 10

**Traits**: **Ambush Predator** - Advantage on its first attack roll if it began the encounter undetected.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Bobcat

Smaller and warier than a mountain lion, same basic approach.

**Soak** 1 · **Attack** 4 · **Defense** 7 · **Health Levels** 3 · **Movement Rate** 10m

**Claws**: 3, Melee

**Notable Skills**: Stealth 9

**Traits**: **Ambush Predator** (as Mountain Lion).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Lynx

Built for cold terrain, hunts the same way a bobcat does.

**Soak** 1 · **Attack** 4 · **Defense** 7 · **Health Levels** 3 · **Movement Rate** 10m

**Claws**: 3, Melee

**Notable Skills**: Stealth 8, Perception 8

**Traits**: **Ambush Predator** (as Mountain Lion).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Black Bear

Strong enough to be dangerous, but genuinely avoidant by nature.

**Soak** 3 · **Attack** 5 · **Defense** 5 · **Health Levels** 7 · **Movement Rate** 9m

**Claws**: 5, Melee

**Notable Skills**: Athletics 8, Perception 7

**Traits**: **Avoidant** - only attacks if cornered, startled at close range, or defending cubs (GM's call).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Grizzly Bear

Nothing about it is avoidant once it's decided to engage.

**Soak** 4 · **Attack** 8 · **Defense** 5 · **Health Levels** 9 · **Movement Rate** 10m

**Claws**: 7, Melee

**Notable Skills**: Athletics 9

**Traits**: **Hard to Put Down** - Advantage on any roll to resist being knocked back, tripped, or grappled.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Polar Bear

The largest land predator that actually hunts people as prey.

**Soak** 4 · **Attack** 8 · **Defense** 5 · **Health Levels** 9 · **Movement Rate** 9m (no penalty in water)

**Claws / Bite**: 7, Melee

**Notable Skills**: Athletics 9, Perception 8 (scent)

**Traits**: **Strong Swimmer** - no Movement Rate penalty in water. **Ambush Predator** (as Mountain Lion) when attacking from open water or through ice.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Rattlesnake

Would rather warn than strike - the warning is the only mercy it offers.

**Soak** 0 · **Attack** 5 · **Defense** 7 · **Health Levels** 1 · **Movement Rate** 3m

**Bite**: 1, Melee, **Venom**: on a successful hit, the target must spend 1 Ki or take Disadvantage on Physical rolls until treated.

**Notable Skills**: Stealth 9

**Traits**: **Warning Display** - always rattles before striking unless it's been stepped on or grabbed directly.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Copperhead

Camouflages well enough that the warning display is easy to miss.

**Soak** 0 · **Attack** 4 · **Defense** 7 · **Health Levels** 1 · **Movement Rate** 3m

**Bite**: 1, Melee, **Venom**: on a successful hit, Disadvantage on the target's next roll.

**Notable Skills**: Stealth 10

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Cottonmouth

Aquatic, territorial, and willing to stand its ground rather than flee into deeper water.

**Soak** 0 · **Attack** 4 · **Defense** 7 · **Health Levels** 1 · **Movement Rate** 3m (6m in water)

**Bite**: 1, Melee, **Venom**: on a successful hit, Disadvantage on the target's next roll.

**Notable Skills**: Stealth 9

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Alligator

Nearly invisible at the waterline until it isn't.

**Soak** 3 · **Attack** 6 · **Defense** 4 · **Health Levels** 6 · **Movement Rate** 6m (12m in water)

**Bite**: 6, Melee

**Notable Skills**: Stealth 9 (lying in wait)

**Traits**: **Death Roll** - on a successful bite, may spend 1 Ki to drag a grabbed target into deep water; what happens next is the GM's call.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Snapping Turtle

Slow, low, and armored - the shell does most of the work.

**Soak** 4 · **Attack** 4 · **Defense** 3 · **Health Levels** 4 · **Movement Rate** 3m

**Bite**: 3, Melee

**Traits**: **Withdraw** - as a Fast action, pulls into its shell, raising Soak to 7 until its next action.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Gila Monster

Slow-moving, but its bite doesn't easily let go.

**Soak** 1 · **Attack** 3 · **Defense** 5 · **Health Levels** 2 · **Movement Rate** 3m

**Bite**: 2, Melee, **Venom / Grip**: a target bitten is also grabbed - freeing them is a contested Potence check against the Gila Monster's own bite grip.

**Notable Skills**: Stealth 6

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Black Widow

Small enough to go unnoticed until the bite already happened.

**Soak** 0 · **Attack** 3 · **Defense** 6 · **Health Levels** 1 · **Movement Rate** 2m

**Bite**: 1, Melee, **Venom**: on a successful hit, the target must spend 1 Ki or take Disadvantage on Physical rolls until treated.

**Notable Skills**: Stealth 10

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Brown Recluse

Its bite rarely hurts in the moment - the real damage comes later.

**Soak** 0 · **Attack** 3 · **Defense** 6 · **Health Levels** 1 · **Movement Rate** 2m

**Bite**: 1, Melee, **Venom**: no immediate Health Level loss, but the wound worsens without treatment over the following days - severity is the GM's call.

**Notable Skills**: Stealth 10

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Wasp Swarm

Individually harmless, collectively a real threat if provoked.

**Soak** 0 · **Attack** 4 · **Defense** 7 · **Health Levels** 2 · **Movement Rate** 6m (flight)

**Stings**: 2, Melee

**Traits**: **Swarm** (as Rat Swarm).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Africanized Bee Swarm

More aggressive and far more persistent than an ordinary hive.

**Soak** 0 · **Attack** 4 · **Defense** 7 · **Health Levels** 3 · **Movement Rate** 6m (flight)

**Stings**: 3, Melee

**Traits**: **Swarm** (as Rat Swarm). **Relentless** - once provoked, pursues a target across several Range Bands rather than breaking off at the hive's edge.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Scorpion

Small, easy to miss, and painful rather than lethal in most cases.

**Soak** 1 · **Attack** 3 · **Defense** 6 · **Health Levels** 1 · **Movement Rate** 4m

**Sting**: 1, Melee, **Venom**: on a successful hit, Disadvantage on the target's next roll.

**Notable Skills**: Stealth 8

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Red-Tailed Hawk

Exceptional eyesight, and a dive that closes distance faster than it looks.

**Soak** 0 · **Attack** 5 · **Defense** 8 · **Health Levels** 2 · **Movement Rate** 15m (flight)

**Talons**: 2, Melee

**Notable Skills**: Perception 10

**Traits**: **Dive Attack** - Advantage on an attack made immediately after diving in from Far range.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Great Horned Owl

Silent wings make it one of the few genuinely hard-to-detect predators in the sky.

**Soak** 0 · **Attack** 5 · **Defense** 8 · **Health Levels** 2 · **Movement Rate** 14m (flight)

**Talons**: 2, Melee

**Notable Skills**: Stealth 9, Perception 9

**Traits**: **Nocturnal Ambush** - Advantage on an attack made in darkness against a target that hasn't detected it.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Turkey Vulture

Not a combat threat under any normal circumstance.

**Soak** 0 · **Attack** 1 · **Defense** 7 · **Health Levels** 2 · **Movement Rate** 12m (flight)

**Beak**: 1, Melee

**Notable Skills**: Perception 9 (smell)

**Traits**: **Scavenger** - avoids conflict entirely unless directly and repeatedly provoked.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Great White Shark

The apex predator of open water, full stop.

**Soak** 3 · **Attack** 7 · **Defense** 5 · **Health Levels** 8 · **Movement Rate** 14m (water only)

**Bite**: 8, Melee

**Notable Skills**: Perception 8

**Traits**: **Ambush Predator** (as Mountain Lion). Cannot leave the water.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Bull Shark

Tolerates brackish and shallow water better than most sharks - which means it turns up closer to shore.

**Soak** 2 · **Attack** 6 · **Defense** 5 · **Health Levels** 6 · **Movement Rate** 12m (water only, including shallows)

**Bite**: 6, Melee

**Notable Skills**: Perception 7

**Traits**: **Ambush Predator** (as Mountain Lion). **Aggressive** - never the one to break off an engagement.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Piranha School

Not a single fish worth worrying about - the school is the threat.

**Soak** 0 · **Attack** 4 · **Defense** 7 · **Health Levels** 3 · **Movement Rate** 10m (water only)

**Bite**: 3, Melee

**Traits**: **Swarm** (as Rat Swarm, water-only). **Frenzy** - once any Health Level damage lands on a target in the water, the school's Bite rises to 5 for the rest of the encounter.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Orca

Intelligent, coordinated, and the only predator on this list that hunts great white sharks.

**Soak** 4 · **Attack** 9 · **Defense** 5 · **Health Levels** 10 · **Movement Rate** 16m (water only)

**Bite / Ram**: 9, Melee

**Notable Skills**: Athletics 9

**Traits**: **Pack Tactics** (as Feral Dog, applies within a pod). **Highly Intelligent** - coordinated tactics beyond a simple attack are the GM's call, not a fixed mechanical bonus.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Electric Eel

Doesn't need to bite when it can simply stop a target from acting.

**Soak** 0 · **Attack** 3 · **Defense** 6 · **Health Levels** 2 · **Movement Rate** 6m (water only)

**Shock**: Close range, no touch required in water. No Health Level cost - a target that fails to resist (GM's call on the resistance roll) is Disadvantaged on all rolls until the end of their next turn.

**Notable Skills**: Stealth 8

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Bison

Placid in a herd, catastrophic when charging.

**Soak** 3 · **Attack** 6 · **Defense** 5 · **Health Levels** 7 · **Movement Rate** 11m

**Gore / Trample**: 5, Melee

**Notable Skills**: Athletics 9

**Traits**: **Charge** (as Bull).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Moose

The largest deer species, and surprisingly willing to fight when provoked.

**Soak** 3 · **Attack** 6 · **Defense** 5 · **Health Levels** 7 · **Movement Rate** 11m

**Antlers / Hooves**: 5, Melee

**Notable Skills**: Athletics 8

**Traits**: **Seasonal Aggression** - genuinely dangerous during rut; avoidant like Black Bear the rest of the year (GM's call on timing).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Elk

Smaller and less consistently aggressive than a moose, same basic shape.

**Soak** 2 · **Attack** 5 · **Defense** 6 · **Health Levels** 5 · **Movement Rate** 12m

**Antlers / Hooves**: 4, Melee

**Notable Skills**: Athletics 8

**Traits**: **Seasonal Aggression** (as Moose, milder).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Arctic Fox

Small, camouflaged, and no real combat threat.

**Soak** 0 · **Attack** 2 · **Defense** 8 · **Health Levels** 2 · **Movement Rate** 10m

**Bite**: 1, Melee

**Notable Skills**: Stealth 9, Perception 8

**Traits**: **Snow Camouflage** - Advantage on Stealth rolls while in snowy terrain.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Snowy Owl

Unlike most owls, hunts by day as often as by night.

**Soak** 0 · **Attack** 4 · **Defense** 8 · **Health Levels** 2 · **Movement Rate** 14m (flight)

**Talons**: 2, Melee

**Notable Skills**: Perception 9

**Traits**: **Ambush Hunter** - Advantage on an attack made against a target that hasn't detected it, day or night (unlike Great Horned Owl, not limited to darkness).

</td></tr></table>

---

## Beast Variants (Dire / Giant / Mutated)

Three reusable variant templates for reskinning any creature in this index, plus a full set of pre-built examples covering all 44 Mundane Beasts above. Apply a template to any creature's printed stats to build a tougher, weirder, or larger version on the fly - the pre-built entries below exist so a GM doesn't have to do that math at the table, not because the math is hard.

**Dire**: Soak +1, Attack +1, Defense -1, Health Levels +3, Movement Rate +2m, Natural Weapon Damage +2.

**Giant**: Soak +2, Attack +1, Defense -2, Health Levels +6, Movement Rate +3m, Natural Weapon Damage +5.

**Mutated**: Soak +1, Attack +1, Defense +0, Health Levels +2, Movement Rate +0, Natural Weapon Damage unchanged - instead, pick one Mutation Trait from the table below.

All modifiers apply to the creature's own printed base stats, not to another variant (a Dire creature is not a valid base for building a Giant version). Every stat stays within the system's existing 1-10 ceiling: Attack and Soak can never exceed 10, Defense never drops below 3, and Natural Weapon Damage never exceeds 10 (the same ceiling weapons.md itself uses) - cap any of these manually if a template pushes a value past its limit. Health Levels and Movement Rate have no cap.

### Mutation Traits

Roll or pick one per Mutated creature - varying the pick keeps a table of mutated animals from feeling like the same creature repeated.

| # | Mutation | Effect |
|---|---|---|
| 1 | **Toxic Bite/Sting** | A target hit by its natural weapon must succeed a Water + Difficulty roll (GM sets Difficulty) or gain a level of Exhausted, until treated with an Antitoxin or First Aid Kit. |
| 2 | **Regenerating** | Heals 1 Health Level at the start of each of its turns. |
| 3 | **Radioactive Touch** | Anyone within Melee range at the start of its turn takes 1 unsoakable Health Level, unless wearing radiation-rated protection (GM's call). |
| 4 | **Extra Limb(s)** | Makes two separate natural-weapon attacks per round instead of one, each its own to-hit roll. |
| 5 | **Chitinous Plating** | Can't be targeted by a called shot (immune to the Armor & Called Shots mechanic's targeting). |
| 6 | **Acidic Blood** | Anyone who lands a Melee hit against it takes 1 unsoakable Health Level from splashback. |
| 7 | **Unnatural Senses** | Advantage on Perception checks in darkness/smoke/concealment; can't be Surprised via Stealth. |
| 8 | **Frenzied Metabolism** | Gains Advantage on its next attack roll whenever it loses a Health Level. |
| 9 | **Camouflaged Growths** | Advantage on Stealth rolls; blends unnaturally well into its environment. |
| 10 | **Toxic Cloud** | Once per encounter, releases a cloud at Close range - anyone inside without protection is at Disadvantage on all rolls for the rest of the Scene (same shape as the Skunk's own Spray ability). |
| 11 | **Explosive Death** | On dropping to 0 Health Levels, deals 3 dice of Physical damage to everyone within Melee range. |
| 12 | **Photophobic Frenzy** | Advantage on attacks made in darkness; Disadvantage on attacks made in direct bright light. |

---

**Domestic & Urban**

<table class="adversary-card"><tr><td markdown="1">

### Dire Feral Dog

Something in the litter came out wrong - bigger jaw, bigger frame, none of the usual skittishness around people.

**Soak** 2 · **Attack** 4 · **Defense** 5 · **Health Levels** 6 · **Movement Rate** 11m

**Bite**: 4, Melee

**Notable Skills**: Perception 8, Athletics 8

**Traits**: **Pack Tactics** - gains Advantage on an attack roll when at least one other canine is also attacking the same target.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Feral Dog

Animal control gets a call about a "bear" and finds this instead, chained to a fence post it long ago outgrew.

**Soak** 3 · **Attack** 4 · **Defense** 4 · **Health Levels** 9 · **Movement Rate** 12m

**Bite**: 7, Melee

**Notable Skills**: Perception 8, Athletics 8

**Traits**: **Pack Tactics** - gains Advantage on an attack roll when at least one other canine is also attacking the same target.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Feral Dog

Every scar on it has already closed by the time you notice it was there.

**Soak** 2 · **Attack** 4 · **Defense** 6 · **Health Levels** 5 · **Movement Rate** 9m

**Bite**: 2, Melee

**Notable Skills**: Perception 8, Athletics 8

**Traits**: **Pack Tactics** - gains Advantage on an attack roll when at least one other canine is also attacking the same target. **Regenerating** - heals 1 Health Level at the start of each of its turns.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Guard Dog

Whoever trained it either didn't know when to stop or wanted exactly this.

**Soak** 2 · **Attack** 5 · **Defense** 5 · **Health Levels** 7 · **Movement Rate** 11m

**Bite**: 5, Melee

**Notable Skills**: Intimidation 8, Perception 9

**Traits**: **Trained to Hold** - immune to being Surprised by a target it's already been set to guard against, and doesn't break off an attack from fear alone.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Guard Dog

A property line marked by a chain-link fence and a shape on the other side that makes the fence look optional.

**Soak** 3 · **Attack** 5 · **Defense** 4 · **Health Levels** 10 · **Movement Rate** 12m

**Bite**: 8, Melee

**Notable Skills**: Intimidation 8, Perception 9

**Traits**: **Trained to Hold** - immune to being Surprised by a target it's already been set to guard against, and doesn't break off an attack from fear alone.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Guard Dog

It stopped needing rest between shifts a while ago, and nobody who feeds it has asked why.

**Soak** 2 · **Attack** 5 · **Defense** 6 · **Health Levels** 6 · **Movement Rate** 9m

**Bite**: 3, Melee

**Notable Skills**: Intimidation 8, Perception 9

**Traits**: **Trained to Hold** - immune to being Surprised by a target it's already been set to guard against, and doesn't break off an attack from fear alone. **Regenerating** - heals 1 Health Level at the start of each of its turns.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Feral Cat

Still fast, still small for its kind, but the claws now leave a wound worth stitches.

**Soak** 1 · **Attack** 3 · **Defense** 7 · **Health Levels** 5 · **Movement Rate** 10m

**Claws**: 3, Melee

**Notable Skills**: Stealth 10, Athletics 9

**Traits**: **Hard to Corner** - climbs and leaps at no penalty; treat any vertical surface or gap within its Movement Rate as normal ground for the purpose of escaping.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Feral Cat

House-cat proportions on something the size of a dog, which is somehow worse than either.

**Soak** 2 · **Attack** 3 · **Defense** 6 · **Health Levels** 8 · **Movement Rate** 11m

**Claws**: 6, Melee

**Notable Skills**: Stealth 10, Athletics 9

**Traits**: **Hard to Corner** - climbs and leaps at no penalty; treat any vertical surface or gap within its Movement Rate as normal ground for the purpose of escaping.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Feral Cat

It sits on a fence rail in plain daylight and takes people a solid minute to actually see.

**Soak** 1 · **Attack** 3 · **Defense** 8 · **Health Levels** 4 · **Movement Rate** 8m

**Claws**: 1, Melee

**Notable Skills**: Stealth 10, Athletics 9

**Traits**: **Hard to Corner** - climbs and leaps at no penalty; treat any vertical surface or gap within its Movement Rate as normal ground for the purpose of escaping. **Camouflaged Growths** - Advantage on Stealth rolls; blends unnaturally well into its environment.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Raccoon

Twice the usual weight and none of the usual hesitation about approaching a lit porch.

**Soak** 2 · **Attack** 4 · **Defense** 6 · **Health Levels** 5 · **Movement Rate** 8m

**Bite / Claws**: 3, Melee

**Notable Skills**: Stealth 8, Perception 7

**Traits**: **Rarely Aggressive** - won't initiate an attack unless cornered, defending young, or rabid. A rabid raccoon gains Advantage on its bite.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Raccoon

Tips over a dumpster instead of digging through it, and doesn't run when the lid clangs.

**Soak** 3 · **Attack** 4 · **Defense** 5 · **Health Levels** 8 · **Movement Rate** 9m

**Bite / Claws**: 6, Melee

**Notable Skills**: Stealth 8, Perception 7

**Traits**: **Rarely Aggressive** - won't initiate an attack unless cornered, defending young, or rabid. A rabid raccoon gains Advantage on its bite.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Raccoon

It works a latch in the dark faster than most people manage in daylight.

**Soak** 2 · **Attack** 4 · **Defense** 7 · **Health Levels** 4 · **Movement Rate** 6m

**Bite / Claws**: 1, Melee

**Notable Skills**: Stealth 8, Perception 7

**Traits**: **Rarely Aggressive** - won't initiate an attack unless cornered, defending young, or rabid. A rabid raccoon gains Advantage on its bite. **Unnatural Senses** - Advantage on Perception checks in darkness/smoke/concealment; can't be Surprised via Stealth.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Skunk

The stripe runs down a body twice the usual length, and the smell carries proportionally.

**Soak** 1 · **Attack** 2 · **Defense** 5 · **Health Levels** 4 · **Movement Rate** 6m

**Spray** (once per encounter, no Health Level cost): the cloud reaches farther and lingers longer than an ordinary skunk's - GM's call on exact range, but Close is a floor, not a ceiling.

**Notable Skills**: Perception 6

**Traits**: **Warning Display** - always gets one clearly-telegraphed round of stomping/hissing before spraying, unless directly attacked first.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Skunk

Nobody's ever gotten close enough to measure it - the smell arrives first and settles the question.

**Soak** 2 · **Attack** 2 · **Defense** 4 · **Health Levels** 7 · **Movement Rate** 7m

**Spray** (once per encounter, no Health Level cost): covers a full Range Band instead of just Close, and the residue is potent enough to ruin gear left nearby.

**Notable Skills**: Perception 6

**Traits**: **Warning Display** - always gets one clearly-telegraphed round of stomping/hissing before spraying, unless directly attacked first.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Skunk

It doesn't wait for the warning stomp anymore - one gland has taken over for the other.

**Soak** 2 · **Attack** 2 · **Defense** 6 · **Health Levels** 3 · **Movement Rate** 4m

**Spray** (once per encounter, no Health Level cost): every target within Close range must wash off or take Disadvantage on all rolls for the rest of the Scene.

**Notable Skills**: Perception 6

**Traits**: **Warning Display** - always gets one clearly-telegraphed round of stomping/hissing before spraying, unless directly attacked first. **Toxic Cloud** - once per encounter, releases a cloud at Close range - anyone inside without protection is at Disadvantage on all rolls for the rest of the Scene (same shape as the Skunk's own Spray ability, and may be triggered in addition to it).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Rat Swarm

Bigger bodies than a rat swarm should have, moving with the same coordination as always.

**Soak** 1 · **Attack** 4 · **Defense** 7 · **Health Levels** 5 · **Movement Rate** 8m

**Bite**: 3, Melee

**Notable Skills**: Stealth 8

**Traits**: **Swarm** - immune to single-target Physical attacks (they connect with mostly empty space); any attack that hits an area instead deals double dice against it.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Rat Swarm

What used to be a hundred small bodies is now a few dozen large ones, and the sound they make is different.

**Soak** 2 · **Attack** 4 · **Defense** 6 · **Health Levels** 8 · **Movement Rate** 9m

**Bite**: 6, Melee

**Notable Skills**: Stealth 8

**Traits**: **Swarm** - immune to single-target Physical attacks (they connect with mostly empty space); any attack that hits an area instead deals double dice against it.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Rat Swarm

Every bite in the mass carries something that outlasts the bite itself.

**Soak** 1 · **Attack** 4 · **Defense** 8 · **Health Levels** 4 · **Movement Rate** 6m

**Bite**: 1, Melee

**Notable Skills**: Stealth 8

**Traits**: **Swarm** - immune to single-target Physical attacks (they connect with mostly empty space); any attack that hits an area instead deals double dice against it. **Toxic Bite/Sting** - a target hit by its bite must succeed a Water + Difficulty roll (GM sets Difficulty) or gain a level of Exhausted, until treated with an Antitoxin or First Aid Kit.

---

</td></tr></table>

**Farm & Rural**

<table class="adversary-card"><tr><td markdown="1">

### Dire Bull

The fence it broke through wasn't the first, and the farmer stopped repairing that one.

**Soak** 4 · **Attack** 7 · **Defense** 4 · **Health Levels** 9 · **Movement Rate** 12m

**Gore**: 7, Melee

**Notable Skills**: Athletics 9

**Traits**: **Charge** - if it moves its full Movement Rate directly at a target before attacking, its Gore rolls one additional die that round.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Bull

A silhouette in the field that people mistake for a shed until it moves.

**Soak** 5 · **Attack** 7 · **Defense** 3 · **Health Levels** 12 · **Movement Rate** 13m

**Gore**: 10, Melee

**Notable Skills**: Athletics 9

**Traits**: **Charge** - if it moves its full Movement Rate directly at a target before attacking, its Gore rolls one additional die that round.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Bull

Fence posts snap clean against its shoulder and it barely slows down.

**Soak** 4 · **Attack** 7 · **Defense** 5 · **Health Levels** 8 · **Movement Rate** 10m

**Gore**: 5, Melee

**Notable Skills**: Athletics 9

**Traits**: **Charge** - if it moves its full Movement Rate directly at a target before attacking, its Gore rolls one additional die that round. **Chitinous Plating** - can't be targeted by a called shot (immune to the Armor & Called Shots mechanic's targeting).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Wild Boar

It's put on enough weight to stop caring what's in its way, tusks included.

**Soak** 3 · **Attack** 6 · **Defense** 4 · **Health Levels** 7 · **Movement Rate** 10m

**Tusks**: 5, Melee

**Notable Skills**: Athletics 8

**Traits**: **Low Profile** - Advantage on any roll to resist being tripped or knocked down.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Wild Boar

Hunters who've seen it describe something closer to a small truck than a hog.

**Soak** 4 · **Attack** 6 · **Defense** 3 · **Health Levels** 10 · **Movement Rate** 11m

**Tusks**: 8, Melee

**Notable Skills**: Athletics 8

**Traits**: **Low Profile** - Advantage on any roll to resist being tripped or knocked down.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Wild Boar

A tusk wound only makes it push harder into whatever caused it.

**Soak** 3 · **Attack** 6 · **Defense** 5 · **Health Levels** 6 · **Movement Rate** 8m

**Tusks**: 3, Melee

**Notable Skills**: Athletics 8

**Traits**: **Low Profile** - Advantage on any roll to resist being tripped or knocked down. **Frenzied Metabolism** - gains Advantage on its next attack roll whenever it loses a Health Level.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Aggressive Goose

Same territorial streak, considerably more weight behind the wing that hits you.

**Soak** 1 · **Attack** 3 · **Defense** 6 · **Health Levels** 4 · **Movement Rate** 8m

**Bite / Wing Buffet**: 3, Melee

**Notable Skills**: Intimidation 7

**Traits**: **Relentless** - keeps attacking after taking damage with no hesitation; only stops if it actually runs out of Health Levels or the target leaves its territory entirely.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Aggressive Goose

Nobody believes the story until they see the pond it now considers its property.

**Soak** 2 · **Attack** 3 · **Defense** 5 · **Health Levels** 7 · **Movement Rate** 9m

**Bite / Wing Buffet**: 6, Melee

**Notable Skills**: Intimidation 7

**Traits**: **Relentless** - keeps attacking after taking damage with no hesitation; only stops if it actually runs out of Health Levels or the target leaves its territory entirely.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Aggressive Goose

It doesn't seem to tire the way a goose should, and the neighbors have stopped mowing near the pond.

**Soak** 1 · **Attack** 3 · **Defense** 7 · **Health Levels** 3 · **Movement Rate** 6m

**Bite / Wing Buffet**: 1, Melee

**Notable Skills**: Intimidation 7

**Traits**: **Relentless** - keeps attacking after taking damage with no hesitation; only stops if it actually runs out of Health Levels or the target leaves its territory entirely. **Regenerating** - heals 1 Health Level at the start of each of its turns.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Spooked Horse

A panicked animal that's outgrown its own stable and still hasn't calmed down.

**Soak** 2 · **Attack** 5 · **Defense** 4 · **Health Levels** 8 · **Movement Rate** 16m

**Kick / Trample**: 6, Melee

**Notable Skills**: Athletics 9

**Traits**: **Trample** - if its movement this round passes through a Close-range space a target occupies, it can make its attack as part of that movement instead of spending a separate action.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Spooked Horse

What starts as a runaway plow horse story usually ends with someone describing something the size of a delivery truck.

**Soak** 3 · **Attack** 5 · **Defense** 3 · **Health Levels** 11 · **Movement Rate** 17m

**Kick / Trample**: 9, Melee

**Notable Skills**: Athletics 9

**Traits**: **Trample** - if its movement this round passes through a Close-range space a target occupies, it can make its attack as part of that movement instead of spending a separate action.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Spooked Horse

It bolts at sounds nothing else in the field even registers, and it hears you coming long before you see it.

**Soak** 2 · **Attack** 5 · **Defense** 5 · **Health Levels** 7 · **Movement Rate** 14m

**Kick / Trample**: 4, Melee

**Notable Skills**: Athletics 9

**Traits**: **Trample** - if its movement this round passes through a Close-range space a target occupies, it can make its attack as part of that movement instead of spending a separate action. **Unnatural Senses** - Advantage on Perception checks in darkness/smoke/concealment; can't be Surprised via Stealth.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Feral Goat

Bigger horns, bigger frame, same willingness to plant its feet and not back down.

**Soak** 2 · **Attack** 4 · **Defense** 5 · **Health Levels** 6 · **Movement Rate** 10m

**Headbutt**: 4, Melee

**Notable Skills**: Athletics 8

**Traits**: **Sure-Footed** - Advantage on any roll to keep its footing on unstable or steep terrain.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Feral Goat

It stands on a rock outcrop that most animals its size couldn't reach, let alone hold.

**Soak** 3 · **Attack** 4 · **Defense** 4 · **Health Levels** 9 · **Movement Rate** 11m

**Headbutt**: 7, Melee

**Notable Skills**: Athletics 8

**Traits**: **Sure-Footed** - Advantage on any roll to keep its footing on unstable or steep terrain.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Feral Goat

Its hide has taken on a texture closer to bark than fur, and a knife doesn't do what it should against it.

**Soak** 2 · **Attack** 4 · **Defense** 6 · **Health Levels** 5 · **Movement Rate** 8m

**Headbutt**: 2, Melee

**Notable Skills**: Athletics 8

**Traits**: **Sure-Footed** - Advantage on any roll to keep its footing on unstable or steep terrain. **Chitinous Plating** - can't be targeted by a called shot (immune to the Armor & Called Shots mechanic's targeting).

---

</td></tr></table>

**Canines**

<table class="adversary-card"><tr><td markdown="1">

### Dire Coyote

Bolder than a coyote has any right to be, and considerably harder to discourage.

**Soak** 2 · **Attack** 4 · **Defense** 6 · **Health Levels** 6 · **Movement Rate** 12m

**Bite**: 4, Melee

**Notable Skills**: Stealth 8, Perception 8

**Traits**: **Pack Tactics** (as Feral Dog).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Coyote

Ranchers who lose stock to it stop calling it a coyote at all.

**Soak** 3 · **Attack** 4 · **Defense** 5 · **Health Levels** 9 · **Movement Rate** 13m

**Bite**: 7, Melee

**Notable Skills**: Stealth 8, Perception 8

**Traits**: **Pack Tactics** (as Feral Dog).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Coyote

It crosses open ground in full moonlight and somehow still isn't the thing you notice first.

**Soak** 2 · **Attack** 4 · **Defense** 7 · **Health Levels** 5 · **Movement Rate** 10m

**Bite**: 2, Melee

**Notable Skills**: Stealth 8, Perception 8

**Traits**: **Pack Tactics** (as Feral Dog). **Camouflaged Growths** - Advantage on Stealth rolls; blends unnaturally well into its environment.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Wolf

Old bones turn up in the region that are too large for any wolf on record, and the tracks nearby are fresh.

**Soak** 3 · **Attack** 6 · **Defense** 5 · **Health Levels** 7 · **Movement Rate** 13m

**Bite**: 6, Melee

**Notable Skills**: Perception 8, Athletics 8

**Traits**: **Pack Tactics** (as Feral Dog) - a wolf attacking a target already engaged by another wolf this Scene gains Advantage even without a second wolf attacking the same round.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Wolf

A pack led by something the size of a horse doesn't behave like a normal pack anymore - it just wins faster.

**Soak** 4 · **Attack** 6 · **Defense** 4 · **Health Levels** 10 · **Movement Rate** 14m

**Bite**: 9, Melee

**Notable Skills**: Perception 8, Athletics 8

**Traits**: **Pack Tactics** (as Feral Dog) - a wolf attacking a target already engaged by another wolf this Scene gains Advantage even without a second wolf attacking the same round.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Wolf

A second, smaller jaw has grown along its flank, and it uses both without missing a beat.

**Soak** 3 · **Attack** 6 · **Defense** 6 · **Health Levels** 6 · **Movement Rate** 11m

**Bite**: 4, Melee

**Notable Skills**: Perception 8, Athletics 8

**Traits**: **Pack Tactics** (as Feral Dog) - a wolf attacking a target already engaged by another wolf this Scene gains Advantage even without a second wolf attacking the same round. **Extra Limb(s)** - makes two separate natural-weapon attacks per round instead of one, each its own to-hit roll.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Wolverine

It was already willing to fight anything - now it's willing to fight anything and win.

**Soak** 4 · **Attack** 7 · **Defense** 4 · **Health Levels** 8 · **Movement Rate** 10m

**Bite / Claws**: 6, Melee

**Notable Skills**: Athletics 8

**Traits**: **Ferocious** - never the one to break off an engagement voluntarily; fights until it or its target is out of Health Levels or the target escapes outright.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Wolverine

Small for a bear and large for a wolverine, which puts it somewhere no one wants to meet in a den.

**Soak** 5 · **Attack** 7 · **Defense** 3 · **Health Levels** 11 · **Movement Rate** 11m

**Bite / Claws**: 9, Melee

**Notable Skills**: Athletics 8

**Traits**: **Ferocious** - never the one to break off an engagement voluntarily; fights until it or its target is out of Health Levels or the target escapes outright.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Wolverine

Every wound just seems to make it angrier, and it was already the wrong animal to make angry.

**Soak** 4 · **Attack** 7 · **Defense** 5 · **Health Levels** 7 · **Movement Rate** 8m

**Bite / Claws**: 4, Melee

**Notable Skills**: Athletics 8

**Traits**: **Ferocious** - never the one to break off an engagement voluntarily; fights until it or its target is out of Health Levels or the target escapes outright. **Frenzied Metabolism** - gains Advantage on its next attack roll whenever it loses a Health Level.

---

</td></tr></table>

**Wild Cats**

<table class="adversary-card"><tr><td markdown="1">

### Dire Mountain Lion

Bigger paws leave bigger prints, and the deer it's been taking down lately are elk-sized.

**Soak** 3 · **Attack** 7 · **Defense** 5 · **Health Levels** 8 · **Movement Rate** 14m

**Claws / Bite**: 7, Melee

**Notable Skills**: Stealth 10, Athletics 10

**Traits**: **Ambush Predator** - Advantage on its first attack roll if it began the encounter undetected.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Mountain Lion

The kind of animal state wildlife agencies quietly stop confirming exists once the reports get consistent.

**Soak** 4 · **Attack** 7 · **Defense** 4 · **Health Levels** 11 · **Movement Rate** 15m

**Claws / Bite**: 10, Melee

**Notable Skills**: Stealth 10, Athletics 10

**Traits**: **Ambush Predator** - Advantage on its first attack roll if it began the encounter undetected.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Mountain Lion

It hears a held breath from across a clearing, which is the only warning anyone gets.

**Soak** 3 · **Attack** 7 · **Defense** 6 · **Health Levels** 7 · **Movement Rate** 12m

**Claws / Bite**: 5, Melee

**Notable Skills**: Stealth 10, Athletics 10

**Traits**: **Ambush Predator** - Advantage on its first attack roll if it began the encounter undetected. **Unnatural Senses** - Advantage on Perception checks in darkness/smoke/concealment; can't be Surprised via Stealth.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Bobcat

Twice the size of a normal bobcat and no less willing to launch from a branch overhead.

**Soak** 2 · **Attack** 5 · **Defense** 6 · **Health Levels** 6 · **Movement Rate** 12m

**Claws**: 5, Melee

**Notable Skills**: Stealth 9

**Traits**: **Ambush Predator** (as Mountain Lion).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Bobcat

Drags a whole deer carcass up a tree with the ease of a normal bobcat carrying a rabbit.

**Soak** 3 · **Attack** 5 · **Defense** 5 · **Health Levels** 9 · **Movement Rate** 13m

**Claws**: 8, Melee

**Notable Skills**: Stealth 9

**Traits**: **Ambush Predator** (as Mountain Lion).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Bobcat

It goes flat against a rock face and simply isn't there anymore, even to a trained eye.

**Soak** 2 · **Attack** 5 · **Defense** 7 · **Health Levels** 5 · **Movement Rate** 10m

**Claws**: 3, Melee

**Notable Skills**: Stealth 9

**Traits**: **Ambush Predator** (as Mountain Lion). **Camouflaged Growths** - Advantage on Stealth rolls; blends unnaturally well into its environment.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Lynx

The wide paws that keep a lynx on top of snow now carry considerably more weight.

**Soak** 2 · **Attack** 5 · **Defense** 6 · **Health Levels** 6 · **Movement Rate** 12m

**Claws**: 5, Melee

**Notable Skills**: Stealth 8, Perception 8

**Traits**: **Ambush Predator** (as Mountain Lion).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Lynx

Tracks in fresh snow that look right in every way except the size.

**Soak** 3 · **Attack** 5 · **Defense** 5 · **Health Levels** 9 · **Movement Rate** 13m

**Claws**: 8, Melee

**Notable Skills**: Stealth 8, Perception 8

**Traits**: **Ambush Predator** (as Mountain Lion).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Lynx

It hunts just as well against a bright snowfield as it does in the dark of a pine stand, though the glare seems to throw it off.

**Soak** 2 · **Attack** 5 · **Defense** 7 · **Health Levels** 5 · **Movement Rate** 10m

**Claws**: 3, Melee

**Notable Skills**: Stealth 8, Perception 8

**Traits**: **Ambush Predator** (as Mountain Lion). **Photophobic Frenzy** - Advantage on attacks made in darkness; Disadvantage on attacks made in direct bright light.

---

</td></tr></table>

**Bears**

<table class="adversary-card"><tr><td markdown="1">

### Dire Black Bear

Stands taller than a grizzly should and still keeps the black bear's habit of avoiding a fight when it can.

**Soak** 4 · **Attack** 6 · **Defense** 4 · **Health Levels** 10 · **Movement Rate** 11m

**Claws**: 7, Melee

**Notable Skills**: Athletics 8, Perception 7

**Traits**: **Avoidant** - only attacks if cornered, startled at close range, or defending cubs (GM's call).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Black Bear

Even avoidant, something this size can knock a car onto its side just by leaning on it wrong.

**Soak** 5 · **Attack** 6 · **Defense** 3 · **Health Levels** 13 · **Movement Rate** 12m

**Claws**: 10, Melee

**Notable Skills**: Athletics 8, Perception 7

**Traits**: **Avoidant** - only attacks if cornered, startled at close range, or defending cubs (GM's call).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Black Bear

A gash that should have laid it up for a season closes over in a matter of hours.

**Soak** 4 · **Attack** 6 · **Defense** 5 · **Health Levels** 9 · **Movement Rate** 9m

**Claws**: 5, Melee

**Notable Skills**: Athletics 8, Perception 7

**Traits**: **Avoidant** - only attacks if cornered, startled at close range, or defending cubs (GM's call). **Regenerating** - heals 1 Health Level at the start of each of its turns.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Grizzly Bear

Already the animal you don't want to surprise - this one just has more mass behind the surprise.

**Soak** 5 · **Attack** 9 · **Defense** 4 · **Health Levels** 12 · **Movement Rate** 12m

**Claws**: 9, Melee

**Notable Skills**: Athletics 9

**Traits**: **Hard to Put Down** - Advantage on any roll to resist being knocked back, tripped, or grappled.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Grizzly Bear

The kind of thing that leaves claw marks on a tree at a height that makes hikers turn around.

**Soak** 6 · **Attack** 9 · **Defense** 3 · **Health Levels** 15 · **Movement Rate** 13m

**Claws**: 10, Melee

**Notable Skills**: Athletics 9

**Traits**: **Hard to Put Down** - Advantage on any roll to resist being knocked back, tripped, or grappled.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Grizzly Bear

Standing near it too long leaves a low ache that won't go away until you've put real distance between you.

**Soak** 5 · **Attack** 9 · **Defense** 5 · **Health Levels** 11 · **Movement Rate** 10m

**Claws**: 7, Melee

**Notable Skills**: Athletics 9

**Traits**: **Hard to Put Down** - Advantage on any roll to resist being knocked back, tripped, or grappled. **Radioactive Touch** - anyone within Melee range at the start of its turn takes 1 unsoakable Health Level, unless wearing radiation-rated protection (GM's call).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Polar Bear

Already the largest thing that hunts people as food - this one just makes the math worse.

**Soak** 5 · **Attack** 9 · **Defense** 4 · **Health Levels** 12 · **Movement Rate** 11m (no penalty in water)

**Claws / Bite**: 9, Melee

**Notable Skills**: Athletics 9, Perception 8 (scent)

**Traits**: **Strong Swimmer** - no Movement Rate penalty in water. **Ambush Predator** (as Mountain Lion) when attacking from open water or through ice.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Polar Bear

Ice researchers who've filmed it from a distance still argue about the scale in the footage.

**Soak** 6 · **Attack** 9 · **Defense** 3 · **Health Levels** 15 · **Movement Rate** 12m (no penalty in water)

**Claws / Bite**: 10, Melee

**Notable Skills**: Athletics 9, Perception 8 (scent)

**Traits**: **Strong Swimmer** - no Movement Rate penalty in water. **Ambush Predator** (as Mountain Lion) when attacking from open water or through ice.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Polar Bear

Anyone who manages to land a hit on it comes away burned as much as bitten.

**Soak** 5 · **Attack** 9 · **Defense** 5 · **Health Levels** 11 · **Movement Rate** 9m (no penalty in water)

**Claws / Bite**: 7, Melee

**Notable Skills**: Athletics 9, Perception 8 (scent)

**Traits**: **Strong Swimmer** - no Movement Rate penalty in water. **Ambush Predator** (as Mountain Lion) when attacking from open water or through ice. **Acidic Blood** - anyone who lands a Melee hit against it takes 1 unsoakable Health Level from splashback.

---

</td></tr></table>

**Reptiles**

<table class="adversary-card"><tr><td markdown="1">

### Dire Rattlesnake

Longer than any rattler has a right to be, and the rattle itself sounds wrong - too deep, too slow.

**Soak** 1 · **Attack** 6 · **Defense** 6 · **Health Levels** 4 · **Movement Rate** 5m

**Bite**: 3, Melee, **Venom**: on a successful hit, the target must spend 1 Ki or take Disadvantage on Physical rolls until treated.

**Notable Skills**: Stealth 9

**Traits**: **Warning Display** - always rattles before striking unless it's been stepped on or grabbed directly.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Rattlesnake

A rockslide of coiled muscle that people mistake for a fallen log until the log moves.

**Soak** 2 · **Attack** 6 · **Defense** 5 · **Health Levels** 7 · **Movement Rate** 6m

**Bite**: 6, Melee, **Venom**: on a successful hit, the target must spend 1 Ki or take Disadvantage on Physical rolls until treated.

**Notable Skills**: Stealth 9

**Traits**: **Warning Display** - always rattles before striking unless it's been stepped on or grabbed directly.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Rattlesnake

One bite carries two different kinds of trouble now, and the second doesn't come with a rattle to warn you.

**Soak** 1 · **Attack** 6 · **Defense** 7 · **Health Levels** 3 · **Movement Rate** 3m

**Bite**: 1, Melee, **Venom**: on a successful hit, the target must spend 1 Ki or take Disadvantage on Physical rolls until treated.

**Notable Skills**: Stealth 9

**Traits**: **Warning Display** - always rattles before striking unless it's been stepped on or grabbed directly. **Toxic Bite/Sting** - a target hit by its bite must also succeed a Water + Difficulty roll (GM sets Difficulty) or gain a level of Exhausted, until treated with an Antitoxin or First Aid Kit.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Copperhead

Its camouflage pattern has stretched with the extra length, and it's harder than ever to spot in leaf litter.

**Soak** 1 · **Attack** 5 · **Defense** 6 · **Health Levels** 4 · **Movement Rate** 5m

**Bite**: 3, Melee, **Venom**: on a successful hit, Disadvantage on the target's next roll.

**Notable Skills**: Stealth 10

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Copperhead

Big enough now to take small game whole, which changes how it hunts more than how it looks.

**Soak** 2 · **Attack** 5 · **Defense** 5 · **Health Levels** 7 · **Movement Rate** 6m

**Bite**: 6, Melee, **Venom**: on a successful hit, Disadvantage on the target's next roll.

**Notable Skills**: Stealth 10

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Copperhead

A second set of fangs has grown behind the first, doubling every strike it lands.

**Soak** 1 · **Attack** 5 · **Defense** 7 · **Health Levels** 3 · **Movement Rate** 3m

**Bite**: 1, Melee, **Venom**: on a successful hit, Disadvantage on the target's next roll.

**Notable Skills**: Stealth 10

**Traits**: **Extra Limb(s)** - makes two separate bite attacks per round instead of one, each its own to-hit roll.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Cottonmouth

It holds its ground in shallow water even against threats that would send an ordinary cottonmouth diving.

**Soak** 1 · **Attack** 5 · **Defense** 6 · **Health Levels** 4 · **Movement Rate** 5m (8m in water)

**Bite**: 3, Melee, **Venom**: on a successful hit, Disadvantage on the target's next roll.

**Notable Skills**: Stealth 9

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Cottonmouth

Fishermen who've hooked something in the shallows and reeled up a snake instead usually don't go back.

**Soak** 2 · **Attack** 5 · **Defense** 5 · **Health Levels** 7 · **Movement Rate** 6m (9m in water)

**Bite**: 6, Melee, **Venom**: on a successful hit, Disadvantage on the target's next roll.

**Notable Skills**: Stealth 9

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Cottonmouth

It can flood a stretch of shallow water with something in the air alone, no bite required.

**Soak** 1 · **Attack** 5 · **Defense** 7 · **Health Levels** 3 · **Movement Rate** 3m (6m in water)

**Bite**: 1, Melee, **Venom**: on a successful hit, Disadvantage on the target's next roll.

**Notable Skills**: Stealth 9

**Traits**: **Toxic Cloud** - once per encounter, releases a cloud at Close range - anyone inside without protection is at Disadvantage on all rolls for the rest of the Scene (same shape as the Skunk's own Spray ability).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Alligator

The waterline hides more of it than usual, simply because there's more to hide.

**Soak** 4 · **Attack** 7 · **Defense** 3 · **Health Levels** 9 · **Movement Rate** 8m (14m in water)

**Bite**: 8, Melee

**Notable Skills**: Stealth 9 (lying in wait)

**Traits**: **Death Roll** - on a successful bite, may spend 1 Ki to drag a grabbed target into deep water; what happens next is the GM's call.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Alligator

Old-timers around the bayou have a name for this one and a firm rule about which stretch of water to avoid.

**Soak** 5 · **Attack** 7 · **Defense** 3 · **Health Levels** 12 · **Movement Rate** 9m (15m in water)

**Bite**: 10, Melee

**Notable Skills**: Stealth 9 (lying in wait)

**Traits**: **Death Roll** - on a successful bite, may spend 1 Ki to drag a grabbed target into deep water; what happens next is the GM's call.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Alligator

Its hide has gone from tough to genuinely impenetrable in places, thick plates where scutes used to be.

**Soak** 4 · **Attack** 7 · **Defense** 4 · **Health Levels** 8 · **Movement Rate** 6m (12m in water)

**Bite**: 6, Melee

**Notable Skills**: Stealth 9 (lying in wait)

**Traits**: **Death Roll** - on a successful bite, may spend 1 Ki to drag a grabbed target into deep water; what happens next is the GM's call. **Chitinous Plating** - can't be targeted by a called shot (immune to the Armor & Called Shots mechanic's targeting).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Snapping Turtle

The shell alone is the size of a manhole cover, and the bite behind it hasn't gotten any gentler.

**Soak** 5 · **Attack** 5 · **Defense** 3 · **Health Levels** 7 · **Movement Rate** 5m

**Bite**: 5, Melee

**Traits**: **Withdraw** - as a Fast action, pulls into its shell, raising Soak to 8 until its next action.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Snapping Turtle

Kayakers give the deep bend in the river a wide berth after word gets around about what's living in it.

**Soak** 6 · **Attack** 5 · **Defense** 3 · **Health Levels** 10 · **Movement Rate** 6m

**Bite**: 8, Melee

**Traits**: **Withdraw** - as a Fast action, pulls into its shell, raising Soak to 9 until its next action.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Snapping Turtle

A cracked shell is back to full thickness within days, no scar to show for it.

**Soak** 5 · **Attack** 5 · **Defense** 3 · **Health Levels** 6 · **Movement Rate** 3m

**Bite**: 3, Melee

**Traits**: **Withdraw** - as a Fast action, pulls into its shell, raising Soak to 7 until its next action. **Regenerating** - heals 1 Health Level at the start of each of its turns.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Gila Monster

It moves no faster than before, but once it clamps down the grip is considerably harder to break.

**Soak** 2 · **Attack** 4 · **Defense** 4 · **Health Levels** 5 · **Movement Rate** 5m

**Bite**: 4, Melee, **Venom / Grip**: a target bitten is also grabbed - freeing them is a contested Potence check against the Gila Monster's own bite grip.

**Notable Skills**: Stealth 6

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Gila Monster

Its coloring is unmistakable even at a distance, which does nothing to help once you're in reach of it.

**Soak** 3 · **Attack** 4 · **Defense** 3 · **Health Levels** 8 · **Movement Rate** 6m

**Bite**: 7, Melee, **Venom / Grip**: a target bitten is also grabbed - freeing them is a contested Potence check against the Gila Monster's own bite grip.

**Notable Skills**: Stealth 6

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Gila Monster

The venom now carries an extra sting that lingers well past the bite itself.

**Soak** 2 · **Attack** 4 · **Defense** 5 · **Health Levels** 4 · **Movement Rate** 3m

**Bite**: 2, Melee, **Venom / Grip**: a target bitten is also grabbed - freeing them is a contested Potence check against the Gila Monster's own bite grip.

**Notable Skills**: Stealth 6

**Traits**: **Toxic Bite/Sting** - a target hit by its bite must also succeed a Water + Difficulty roll (GM sets Difficulty) or gain a level of Exhausted, until treated with an Antitoxin or First Aid Kit.

---

</td></tr></table>

**Arachnids & Insects**

<table class="adversary-card"><tr><td markdown="1">

### Dire Black Widow

The hourglass marking is now visible from across a room, on a body that shouldn't fit in most corners anymore.

**Soak** 1 · **Attack** 4 · **Defense** 5 · **Health Levels** 4 · **Movement Rate** 4m

**Bite**: 3, Melee, **Venom**: on a successful hit, the target must spend 1 Ki or take Disadvantage on Physical rolls until treated.

**Notable Skills**: Stealth 10

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Black Widow

Its web spans a doorway rather than a corner, strong enough to catch more than insects.

**Soak** 2 · **Attack** 4 · **Defense** 4 · **Health Levels** 7 · **Movement Rate** 5m

**Bite**: 6, Melee, **Venom**: on a successful hit, the target must spend 1 Ki or take Disadvantage on Physical rolls until treated.

**Notable Skills**: Stealth 10

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Black Widow

Crushing it doesn't end the encounter cleanly - whatever's in its body finds its way onto whoever did the crushing.

**Soak** 1 · **Attack** 4 · **Defense** 6 · **Health Levels** 3 · **Movement Rate** 2m

**Bite**: 1, Melee, **Venom**: on a successful hit, the target must spend 1 Ki or take Disadvantage on Physical rolls until treated.

**Notable Skills**: Stealth 10

**Traits**: **Acidic Blood** - anyone who lands a Melee hit against it takes 1 unsoakable Health Level from splashback.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Brown Recluse

The bite is the same shape as always, just delivered by something considerably harder to miss.

**Soak** 1 · **Attack** 4 · **Defense** 5 · **Health Levels** 4 · **Movement Rate** 4m

**Bite**: 3, Melee, **Venom**: no immediate Health Level loss, but the wound worsens without treatment over the following days - severity is the GM's call.

**Notable Skills**: Stealth 10

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Brown Recluse

Found under boards in an old shed, in a size that makes the whole shed worth avoiding.

**Soak** 2 · **Attack** 4 · **Defense** 4 · **Health Levels** 7 · **Movement Rate** 5m

**Bite**: 6, Melee, **Venom**: no immediate Health Level loss, but the wound worsens without treatment over the following days - severity is the GM's call.

**Notable Skills**: Stealth 10

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Brown Recluse

Whatever it carries in its bite now works faster and meaner than the necrosis it's known for.

**Soak** 1 · **Attack** 4 · **Defense** 6 · **Health Levels** 3 · **Movement Rate** 2m

**Bite**: 1, Melee, **Venom**: no immediate Health Level loss, but the wound worsens without treatment over the following days - severity is the GM's call.

**Notable Skills**: Stealth 10

**Traits**: **Toxic Bite/Sting** - a target hit by its bite must also succeed a Water + Difficulty roll (GM sets Difficulty) or gain a level of Exhausted, until treated with an Antitoxin or First Aid Kit.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Wasp Swarm

Individual wasps the size of a thumb, still moving and stinging as one body.

**Soak** 1 · **Attack** 5 · **Defense** 6 · **Health Levels** 5 · **Movement Rate** 8m (flight)

**Stings**: 4, Melee

**Traits**: **Swarm** (as Rat Swarm).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Wasp Swarm

The nest that produced it is the size of a car, and nobody's gone back to check on it twice.

**Soak** 2 · **Attack** 5 · **Defense** 5 · **Health Levels** 8 · **Movement Rate** 9m (flight)

**Stings**: 7, Melee

**Traits**: **Swarm** (as Rat Swarm).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Wasp Swarm

Every wasp in the mass carries two stingers instead of one, and both find their mark.

**Soak** 1 · **Attack** 5 · **Defense** 7 · **Health Levels** 4 · **Movement Rate** 6m (flight)

**Stings**: 2, Melee

**Traits**: **Swarm** (as Rat Swarm). **Extra Limb(s)** - makes two separate Sting attacks per round instead of one, each its own to-hit roll.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Africanized Bee Swarm

Already more aggressive than a normal hive, and now large enough to blot out a doorway.

**Soak** 1 · **Attack** 5 · **Defense** 6 · **Health Levels** 6 · **Movement Rate** 8m (flight)

**Stings**: 5, Melee

**Traits**: **Swarm** (as Rat Swarm). **Relentless** - once provoked, pursues a target across several Range Bands rather than breaking off at the hive's edge.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Africanized Bee Swarm

The hum alone is audible from a full block away, and it doesn't stop at property lines.

**Soak** 2 · **Attack** 5 · **Defense** 5 · **Health Levels** 9 · **Movement Rate** 9m (flight)

**Stings**: 8, Melee

**Traits**: **Swarm** (as Rat Swarm). **Relentless** - once provoked, pursues a target across several Range Bands rather than breaking off at the hive's edge.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Africanized Bee Swarm

It carries a payload beyond the sting now, one it can loose all at once against anyone standing too close.

**Soak** 1 · **Attack** 5 · **Defense** 7 · **Health Levels** 5 · **Movement Rate** 6m (flight)

**Stings**: 3, Melee

**Traits**: **Swarm** (as Rat Swarm). **Relentless** - once provoked, pursues a target across several Range Bands rather than breaking off at the hive's edge. **Toxic Cloud** - once per encounter, releases a cloud at Close range - anyone inside without protection is at Disadvantage on all rolls for the rest of the Scene (same shape as the Skunk's own Spray ability).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Scorpion

Big enough now that the tail arcing overhead is the first thing anyone notices about it.

**Soak** 2 · **Attack** 4 · **Defense** 5 · **Health Levels** 4 · **Movement Rate** 6m

**Sting**: 3, Melee, **Venom**: on a successful hit, Disadvantage on the target's next roll.

**Notable Skills**: Stealth 8

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Scorpion

It doesn't hide under a rock anymore - there isn't a rock big enough.

**Soak** 3 · **Attack** 4 · **Defense** 4 · **Health Levels** 7 · **Movement Rate** 7m

**Sting**: 6, Melee, **Venom**: on a successful hit, Disadvantage on the target's next roll.

**Notable Skills**: Stealth 8

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Scorpion

The exoskeleton has thickened into something closer to armor plate, and a blade skates off it more often than not.

**Soak** 2 · **Attack** 4 · **Defense** 6 · **Health Levels** 3 · **Movement Rate** 4m

**Sting**: 1, Melee, **Venom**: on a successful hit, Disadvantage on the target's next roll.

**Notable Skills**: Stealth 8

**Traits**: **Chitinous Plating** - can't be targeted by a called shot (immune to the Armor & Called Shots mechanic's targeting).

---

</td></tr></table>

**Birds of Prey & Scavengers**

<table class="adversary-card"><tr><td markdown="1">

### Dire Red-Tailed Hawk

Wingspan closer to a small aircraft, and the dive it drops into hits with a lot more behind it.

**Soak** 1 · **Attack** 6 · **Defense** 7 · **Health Levels** 5 · **Movement Rate** 17m (flight)

**Talons**: 4, Melee

**Notable Skills**: Perception 10

**Traits**: **Dive Attack** - Advantage on an attack made immediately after diving in from Far range.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Red-Tailed Hawk

Farmers report livestock disappearing from open pasture in broad daylight, no drag marks anywhere.

**Soak** 2 · **Attack** 6 · **Defense** 6 · **Health Levels** 8 · **Movement Rate** 18m (flight)

**Talons**: 7, Melee

**Notable Skills**: Perception 10

**Traits**: **Dive Attack** - Advantage on an attack made immediately after diving in from Far range.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Red-Tailed Hawk

It picks out movement through fog and brush that would hide it from any normal set of eyes.

**Soak** 1 · **Attack** 6 · **Defense** 8 · **Health Levels** 4 · **Movement Rate** 15m (flight)

**Talons**: 2, Melee

**Notable Skills**: Perception 10

**Traits**: **Dive Attack** - Advantage on an attack made immediately after diving in from Far range. **Unnatural Senses** - Advantage on Perception checks in darkness/smoke/concealment; can't be Surprised via Stealth.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Great Horned Owl

Still silent on the wing, which is somehow more unsettling given how much bigger the wing has gotten.

**Soak** 1 · **Attack** 6 · **Defense** 7 · **Health Levels** 5 · **Movement Rate** 16m (flight)

**Talons**: 4, Melee

**Notable Skills**: Stealth 9, Perception 9

**Traits**: **Nocturnal Ambush** - Advantage on an attack made in darkness against a target that hasn't detected it.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Great Horned Owl

A shape passes overhead at dusk large enough to blot out the last of the light.

**Soak** 2 · **Attack** 6 · **Defense** 6 · **Health Levels** 8 · **Movement Rate** 17m (flight)

**Talons**: 7, Melee

**Notable Skills**: Stealth 9, Perception 9

**Traits**: **Nocturnal Ambush** - Advantage on an attack made in darkness against a target that hasn't detected it.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Great Horned Owl

Daylight seems to throw off its aim in a way darkness never does.

**Soak** 1 · **Attack** 6 · **Defense** 8 · **Health Levels** 4 · **Movement Rate** 14m (flight)

**Talons**: 2, Melee

**Notable Skills**: Stealth 9, Perception 9

**Traits**: **Nocturnal Ambush** - Advantage on an attack made in darkness against a target that hasn't detected it. **Photophobic Frenzy** - Advantage on attacks made in darkness; Disadvantage on attacks made in direct bright light.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Turkey Vulture

Circles higher and stays longer than a normal vulture would, patient in a way that reads as deliberate.

**Soak** 1 · **Attack** 2 · **Defense** 6 · **Health Levels** 5 · **Movement Rate** 14m (flight)

**Beak**: 3, Melee

**Notable Skills**: Perception 9 (smell)

**Traits**: **Scavenger** - avoids conflict entirely unless directly and repeatedly provoked.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Turkey Vulture

A wingspan wide enough to cast a shadow you notice before you notice the bird.

**Soak** 2 · **Attack** 2 · **Defense** 5 · **Health Levels** 8 · **Movement Rate** 15m (flight)

**Beak**: 6, Melee

**Notable Skills**: Perception 9 (smell)

**Traits**: **Scavenger** - avoids conflict entirely unless directly and repeatedly provoked.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Turkey Vulture

Even provoked, it rarely bites - it lets whatever's already wrong with its stomach do the work for it.

**Soak** 1 · **Attack** 2 · **Defense** 7 · **Health Levels** 4 · **Movement Rate** 12m (flight)

**Beak**: 1, Melee

**Notable Skills**: Perception 9 (smell)

**Traits**: **Scavenger** - avoids conflict entirely unless directly and repeatedly provoked. **Acidic Blood** - anyone who lands a Melee hit against it takes 1 unsoakable Health Level from splashback.

---

</td></tr></table>

**Aquatic**

<table class="adversary-card"><tr><td markdown="1">

### Dire Great White Shark

Boat captains who've clocked its length on sonar stop taking clients out past the point.

**Soak** 4 · **Attack** 8 · **Defense** 4 · **Health Levels** 11 · **Movement Rate** 16m (water only)

**Bite**: 10, Melee

**Notable Skills**: Perception 8

**Traits**: **Ambush Predator** (as Mountain Lion). Cannot leave the water.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Great White Shark

The apex predator of open water just got a size class nothing else in that water can match.

**Soak** 5 · **Attack** 8 · **Defense** 3 · **Health Levels** 14 · **Movement Rate** 17m (water only)

**Bite**: 10, Melee

**Notable Skills**: Perception 8

**Traits**: **Ambush Predator** (as Mountain Lion). Cannot leave the water.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Great White Shark

The first taste of blood in the water only sharpens whatever comes next.

**Soak** 4 · **Attack** 8 · **Defense** 5 · **Health Levels** 10 · **Movement Rate** 14m (water only)

**Bite**: 8, Melee

**Notable Skills**: Perception 8

**Traits**: **Ambush Predator** (as Mountain Lion). Cannot leave the water. **Frenzied Metabolism** - gains Advantage on its next attack roll whenever it loses a Health Level.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Bull Shark

Comfortable in the same shallow water it always was, just a great deal harder to miss now.

**Soak** 3 · **Attack** 7 · **Defense** 4 · **Health Levels** 9 · **Movement Rate** 14m (water only, including shallows)

**Bite**: 8, Melee

**Notable Skills**: Perception 7

**Traits**: **Ambush Predator** (as Mountain Lion). **Aggressive** - never the one to break off an engagement.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Bull Shark

Turns up in a river channel miles from the coast, in water that shouldn't hold anything this size.

**Soak** 4 · **Attack** 7 · **Defense** 3 · **Health Levels** 12 · **Movement Rate** 15m (water only, including shallows)

**Bite**: 10, Melee

**Notable Skills**: Perception 7

**Traits**: **Ambush Predator** (as Mountain Lion). **Aggressive** - never the one to break off an engagement.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Bull Shark

A second row of fins has grown along its underside, letting it turn and strike in the same motion.

**Soak** 3 · **Attack** 7 · **Defense** 5 · **Health Levels** 8 · **Movement Rate** 12m (water only, including shallows)

**Bite**: 6, Melee

**Notable Skills**: Perception 7

**Traits**: **Ambush Predator** (as Mountain Lion). **Aggressive** - never the one to break off an engagement. **Extra Limb(s)** - makes two separate Bite attacks per round instead of one, each its own to-hit roll.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Piranha School

The individual fish have grown past what any aquarium description would call a piranha.

**Soak** 1 · **Attack** 5 · **Defense** 6 · **Health Levels** 6 · **Movement Rate** 12m (water only)

**Bite**: 5, Melee

**Traits**: **Swarm** (as Rat Swarm, water-only). **Frenzy** - once any Health Level damage lands on a target in the water, the school's Bite rises to 7 for the rest of the encounter.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Piranha School

A stretch of river that strips a carcass to bone in under a minute, no matter the size of the carcass.

**Soak** 2 · **Attack** 5 · **Defense** 5 · **Health Levels** 9 · **Movement Rate** 13m (water only)

**Bite**: 8, Melee

**Traits**: **Swarm** (as Rat Swarm, water-only). **Frenzy** - once any Health Level damage lands on a target in the water, the school's Bite rises to 10 for the rest of the encounter.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Piranha School

Bites that should have killed half the school instead seem to close over within the hour.

**Soak** 1 · **Attack** 5 · **Defense** 7 · **Health Levels** 5 · **Movement Rate** 10m (water only)

**Bite**: 3, Melee

**Traits**: **Swarm** (as Rat Swarm, water-only). **Frenzy** - once any Health Level damage lands on a target in the water, the school's Bite rises to 5 for the rest of the encounter. **Regenerating** - heals 1 Health Level at the start of each of its turns.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Orca

Even among orcas this one stands out, and the pod defers to it without hesitation.

**Soak** 5 · **Attack** 10 · **Defense** 4 · **Health Levels** 13 · **Movement Rate** 18m (water only)

**Bite / Ram**: 10, Melee

**Notable Skills**: Athletics 9

**Traits**: **Pack Tactics** (as Feral Dog, applies within a pod). **Highly Intelligent** - coordinated tactics beyond a simple attack are the GM's call, not a fixed mechanical bonus.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Orca

The only predator that hunts great white sharks, now scaled up to a size that makes the comparison almost unfair.

**Soak** 6 · **Attack** 10 · **Defense** 3 · **Health Levels** 16 · **Movement Rate** 19m (water only)

**Bite / Ram**: 10, Melee

**Notable Skills**: Athletics 9

**Traits**: **Pack Tactics** (as Feral Dog, applies within a pod). **Highly Intelligent** - coordinated tactics beyond a simple attack are the GM's call, not a fixed mechanical bonus.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Orca

It tracks a boat's engine noise from a distance no sonar operator expects, and reacts to it before anyone sees a fin.

**Soak** 5 · **Attack** 10 · **Defense** 5 · **Health Levels** 12 · **Movement Rate** 16m (water only)

**Bite / Ram**: 9, Melee

**Notable Skills**: Athletics 9

**Traits**: **Pack Tactics** (as Feral Dog, applies within a pod). **Highly Intelligent** - coordinated tactics beyond a simple attack are the GM's call, not a fixed mechanical bonus. **Unnatural Senses** - Advantage on Perception checks in darkness/smoke/concealment; can't be Surprised via Stealth.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Electric Eel

Longer than any recorded specimen, and the charge it puts out lasts noticeably longer too.

**Soak** 1 · **Attack** 4 · **Defense** 5 · **Health Levels** 5 · **Movement Rate** 8m (water only)

**Shock**: Close range, no touch required in water. No Health Level cost - a target that fails to resist (GM's call on the resistance roll) is Disadvantaged on all rolls for a full Scene instead of just until the end of their next turn.

**Notable Skills**: Stealth 8

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Electric Eel

Whole sections of a flooded channel go dead quiet around it, fish and birds alike giving it a wide berth.

**Soak** 2 · **Attack** 4 · **Defense** 4 · **Health Levels** 8 · **Movement Rate** 9m (water only)

**Shock**: extends out to Short range instead of just Close, no touch required in water. No Health Level cost - a target that fails to resist (GM's call on the resistance roll) is Disadvantaged on all rolls until the end of their next turn.

**Notable Skills**: Stealth 8

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Electric Eel

Even out of the water, standing near it too long leaves a faint prickling under the skin.

**Soak** 1 · **Attack** 4 · **Defense** 6 · **Health Levels** 4 · **Movement Rate** 6m (water only)

**Shock**: Close range, no touch required in water. No Health Level cost - a target that fails to resist (GM's call on the resistance roll) is Disadvantaged on all rolls until the end of their next turn.

**Notable Skills**: Stealth 8

**Traits**: **Radioactive Touch** - anyone within Melee range at the start of its turn takes 1 unsoakable Health Level, unless wearing radiation-rated protection (GM's call).

---

</td></tr></table>

**Large Herbivores**

<table class="adversary-card"><tr><td markdown="1">

### Dire Bison

A herd animal that's outgrown the herd, and it doesn't wait for the rest to catch up before charging.

**Soak** 4 · **Attack** 7 · **Defense** 4 · **Health Levels** 10 · **Movement Rate** 13m

**Gore / Trample**: 7, Melee

**Notable Skills**: Athletics 9

**Traits**: **Charge** (as Bull).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Bison

The ground shakes before it's even visible over the ridge line.

**Soak** 5 · **Attack** 7 · **Defense** 3 · **Health Levels** 13 · **Movement Rate** 14m

**Gore / Trample**: 10, Melee

**Notable Skills**: Athletics 9

**Traits**: **Charge** (as Bull).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Bison

Dropping it doesn't end the danger - what happens next is best watched from well outside Melee range.

**Soak** 4 · **Attack** 7 · **Defense** 5 · **Health Levels** 9 · **Movement Rate** 11m

**Gore / Trample**: 5, Melee

**Notable Skills**: Athletics 9

**Traits**: **Charge** (as Bull). **Explosive Death** - on dropping to 0 Health Levels, deals 3 dice of Physical damage to everyone within Melee range.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Moose

The rack alone is wider than most doorframes, and it's just as willing to fight as an ordinary moose in rut.

**Soak** 4 · **Attack** 7 · **Defense** 4 · **Health Levels** 10 · **Movement Rate** 13m

**Antlers / Hooves**: 7, Melee

**Notable Skills**: Athletics 8

**Traits**: **Seasonal Aggression** - genuinely dangerous during rut; avoidant like Black Bear the rest of the year (GM's call on timing).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Moose

Drivers who've hit a normal moose describe the damage as bad. Nobody's walked away from hitting this one.

**Soak** 5 · **Attack** 7 · **Defense** 3 · **Health Levels** 13 · **Movement Rate** 14m

**Antlers / Hooves**: 10, Melee

**Notable Skills**: Athletics 8

**Traits**: **Seasonal Aggression** - genuinely dangerous during rut; avoidant like Black Bear the rest of the year (GM's call on timing).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Moose

The hide over its chest and flanks has hardened into something closer to bark than skin.

**Soak** 4 · **Attack** 7 · **Defense** 5 · **Health Levels** 9 · **Movement Rate** 11m

**Antlers / Hooves**: 5, Melee

**Notable Skills**: Athletics 8

**Traits**: **Seasonal Aggression** - genuinely dangerous during rut; avoidant like Black Bear the rest of the year (GM's call on timing). **Chitinous Plating** - can't be targeted by a called shot (immune to the Armor & Called Shots mechanic's targeting).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Elk

Bigger, louder during rut, and considerably less likely to back off from a road or a campsite.

**Soak** 3 · **Attack** 6 · **Defense** 5 · **Health Levels** 8 · **Movement Rate** 14m

**Antlers / Hooves**: 6, Melee

**Notable Skills**: Athletics 8

**Traits**: **Seasonal Aggression** (as Moose, milder).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Elk

A rack that would need its own trailer, on an animal that's stopped being camera-shy about it.

**Soak** 4 · **Attack** 6 · **Defense** 4 · **Health Levels** 11 · **Movement Rate** 15m

**Antlers / Hooves**: 9, Melee

**Notable Skills**: Athletics 8

**Traits**: **Seasonal Aggression** (as Moose, milder).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Elk

It stands motionless at the treeline for so long that most people walk right past without ever clocking it.

**Soak** 3 · **Attack** 6 · **Defense** 6 · **Health Levels** 7 · **Movement Rate** 12m

**Antlers / Hooves**: 4, Melee

**Notable Skills**: Athletics 8

**Traits**: **Seasonal Aggression** (as Moose, milder). **Camouflaged Growths** - Advantage on Stealth rolls; blends unnaturally well into its environment.

---

</td></tr></table>

**Arctic Specialists**

<table class="adversary-card"><tr><td markdown="1">

### Dire Arctic Fox

Bigger than the terrain usually allows, though the coat and the caution are exactly the same.

**Soak** 1 · **Attack** 3 · **Defense** 7 · **Health Levels** 5 · **Movement Rate** 12m

**Bite**: 3, Melee

**Notable Skills**: Stealth 9, Perception 8

**Traits**: **Snow Camouflage** - Advantage on Stealth rolls while in snowy terrain.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Arctic Fox

Tracks in fresh snow the size of a wolf's, from an animal that's never been mistaken for one before.

**Soak** 2 · **Attack** 3 · **Defense** 6 · **Health Levels** 8 · **Movement Rate** 13m

**Bite**: 6, Melee

**Notable Skills**: Stealth 9, Perception 8

**Traits**: **Snow Camouflage** - Advantage on Stealth rolls while in snowy terrain.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Arctic Fox

Against a snowfield it simply isn't there until it wants to be, coat and all.

**Soak** 1 · **Attack** 3 · **Defense** 8 · **Health Levels** 4 · **Movement Rate** 10m

**Bite**: 1, Melee

**Notable Skills**: Stealth 9, Perception 8

**Traits**: **Snow Camouflage** - Advantage on Stealth rolls while in snowy terrain. **Camouflaged Growths** - Advantage on Stealth rolls; blends unnaturally well into its environment.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dire Snowy Owl

The wingspan alone casts a shadow across the snow that most prey mistake for a cloud passing over.

**Soak** 1 · **Attack** 5 · **Defense** 7 · **Health Levels** 5 · **Movement Rate** 16m (flight)

**Talons**: 4, Melee

**Notable Skills**: Perception 9

**Traits**: **Ambush Hunter** - Advantage on an attack made against a target that hasn't detected it, day or night (unlike Great Horned Owl, not limited to darkness).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Giant Snowy Owl

Research stations that track it by transponder alone still don't believe the wingspan readings.

**Soak** 2 · **Attack** 5 · **Defense** 6 · **Health Levels** 8 · **Movement Rate** 17m (flight)

**Talons**: 7, Melee

**Notable Skills**: Perception 9

**Traits**: **Ambush Hunter** - Advantage on an attack made against a target that hasn't detected it, day or night (unlike Great Horned Owl, not limited to darkness).

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mutated Snowy Owl

It hunts equally well against the glare of open snow at noon or in the dead dark of a polar night, though never both in the same breath.

**Soak** 1 · **Attack** 5 · **Defense** 8 · **Health Levels** 4 · **Movement Rate** 14m (flight)

**Talons**: 2, Melee

**Notable Skills**: Perception 9

**Traits**: **Ambush Hunter** - Advantage on an attack made against a target that hasn't detected it, day or night (unlike Great Horned Owl, not limited to darkness). **Photophobic Frenzy** - Advantage on attacks made in darkness; Disadvantage on attacks made in direct bright light.

</td></tr></table>

---

## Cryptids

American folklore cryptids, treated as real but rare in a modern setting. Sightings are genuine, evidence is thin, and most of these would rather not be found.

**Cryptids**: Sasquatch · Chupacabra · Mothman · Jersey Devil · Lake Monster · Thunderbird · Dogman · Hodag · Fresno Nightcrawler · Flatwoods Monster · Beast of Bray Road · Loveland Frogman · Black-Eyed Children · Hopkinsville Goblins · Jackalope · Pope Lick Monster · Beast of Bladenboro · Ozark Howler

---

<table class="adversary-card"><tr><td markdown="1">

### Sasquatch

Big enough that most encounters end with it walking away rather than fighting - the stories about the ones that didn't are the ones that get told.

**Soak** 3 · **Attack** 6 · **Defense** 5 · **Health Levels** 7 · **Movement Rate** 10m

**Fists / Thrown Debris**: 5, Melee or Close

**Notable Skills**: Stealth 9, Athletics 9

**Traits**: **Avoidant** - won't attack unless cornered, wounded, or defending young. **Immense Strength** - Advantage on any roll to break a grapple, force open an obstruction, or throw something heavy.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Chupacabra

Ranchers lose livestock to it long before anyone gets a clear look.

**Soak** 1 · **Attack** 5 · **Defense** 7 · **Health Levels** 3 · **Movement Rate** 9m

**Bite / Talons**: 3, Melee

**Notable Skills**: Stealth 9, Perception 7

**Traits**: **Livestock Predator** - Advantage on attacks against domestic and farm animals, and on Stealth rolls made while moving through a pen or pasture at night.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mothman

Shows up before something goes wrong, or that's how the stories always get told after the fact.

**Soak** 1 · **Attack** 4 · **Defense** 8 · **Health Levels** 4 · **Movement Rate** 16m (flight)

**Claws**: 2, Melee (rarely used)

**Notable Skills**: Stealth 8, Perception 10

**Traits**: **Ill Omen** - anyone who gets a clear, sustained look at it takes Disadvantage on their next roll, unnerved rather than harmed. **Avoidant** - flees at the first sign of real conflict and almost never initiates one.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Jersey Devil

Hooved, winged, and loud about it - the scream carries farther than the sighting ever does.

**Soak** 2 · **Attack** 5 · **Defense** 7 · **Health Levels** 5 · **Movement Rate** 14m (flight)

**Claws / Bite**: 4, Melee

**Notable Skills**: Stealth 8, Intimidation 8

**Traits**: **Shriek** (once per encounter) - everyone within Close range takes Disadvantage on their next Perception roll, startled badly enough to lose track of where it went.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Lake Monster

Every deep, cold lake seems to have one story like it. This is the generic write-up; rename it to whatever local legend fits the table.

**Soak** 4 · **Attack** 6 · **Defense** 4 · **Health Levels** 8 · **Movement Rate** 10m (water only)

**Bite / Crush**: 6, Melee

**Traits**: **Ambush from Depth** - Advantage on its first attack if it was submerged and undetected beforehand. Cannot leave the water.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Thunderbird

Wingspan alone is enough to convince most witnesses they saw something that shouldn't exist.

**Soak** 3 · **Attack** 7 · **Defense** 6 · **Health Levels** 7 · **Movement Rate** 18m (flight)

**Talons**: 6, Melee

**Notable Skills**: Perception 9

**Traits**: **Storm Cover** - Advantage on Stealth while flying through cloud or storm cover, and Perception rolls made to track it at range take Disadvantage in poor weather.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Dogman

Upright when it wants distance covered fast, on all fours when it wants to close in quiet.

**Soak** 2 · **Attack** 6 · **Defense** 6 · **Health Levels** 5 · **Movement Rate** 12m

**Claws / Bite**: 5, Melee

**Notable Skills**: Athletics 9, Stealth 8

**Traits**: **Two Gaits** - may drop to all fours as part of its movement for an extra 4m that round, at a cost of -1 Defense until its next action.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Hodag

Squat, spined, and stubborn about its territory - the Wisconsin logging camps that first told stories about it weren't exaggerating the attitude.

**Soak** 3 · **Attack** 4 · **Defense** 4 · **Health Levels** 5 · **Movement Rate** 5m

**Spines / Bite**: 4, Melee

**Traits**: **Den Ambush** - Advantage on the first attack against anything that enters Close range of its den.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Fresno Nightcrawler

Two legs, no visible arms or head, and no recorded case of it ever doing anything but walk away.

**Soak** 0 · **Attack** 1 · **Defense** 9 · **Health Levels** 2 · **Movement Rate** 4m

**Notable Skills**: Stealth 10

**Traits**: **Utterly Nonaggressive** - never attacks under any circumstance. This entry exists for encounters built on observation and unease, not combat.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Flatwoods Monster

Hisses, hovers, and leaves behind a smell that lingers for hours after it's gone.

**Soak** 1 · **Attack** 3 · **Defense** 7 · **Health Levels** 3 · **Movement Rate** 6m (glides)

**Noxious Mist** (once per encounter, no Health Level cost): everyone within Close range must resist or take Disadvantage on Physical rolls for the rest of the Scene.

**Notable Skills**: Stealth 8

**Traits**: **Avoidant** - retreats into its own mist the moment it's directly engaged.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Beast of Bray Road

Roadside sightings, always at night, always gone by the time headlights swing back around.

**Soak** 2 · **Attack** 6 · **Defense** 6 · **Health Levels** 4 · **Movement Rate** 12m

**Claws / Bite**: 4, Melee

**Notable Skills**: Perception 8, Athletics 8

**Traits**: **Roadside Ambush** - Advantage on an attack made against a target it approached along a road, fence line, or open field after dark.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Loveland Frogman

Reeds and riverbanks hide it well enough that most reports are just a shape and a splash.

**Soak** 1 · **Attack** 4 · **Defense** 6 · **Health Levels** 3 · **Movement Rate** 8m (12m in water)

**Claws**: 3, Melee

**Notable Skills**: Stealth 8, Athletics 8

**Traits**: **Leaping Ambush** - Advantage on its first attack if it leaps from water or heavy reeds at a target within Close range.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Black-Eyed Children

They knock, they ask to come in, and the wrongness is entirely in how ordinary the request sounds coming from something that looks at you like that.

**Soak** 0 · **Attack** 2 · **Defense** 6 · **Health Levels** 2 · **Movement Rate** 8m

**Notable Skills**: Persuasion 9

**Traits**: **Wrong in a Way You Can't Name** - anyone who holds eye contact with one, or lets one inside, takes Disadvantage on their next roll. That's the full extent of what they do; if actually attacked, they flee rather than fight.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Hopkinsville Goblins

Small, pale, all-limbs humanoids that come out at night in numbers and don't scare easy.

**Soak** 0 · **Attack** 3 · **Defense** 6 · **Health Levels** 1 · **Movement Rate** 6m

**Claws / Small Blades**: 2, Melee

**Notable Skills**: Stealth 9

**Traits**: **Pack Tactics** (as Feral Dog). **Floats Oddly** - Advantage on Stealth while moving along rooflines, fences, or low structures.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Jackalope

The joke wears off the first time one actually charges.

**Soak** 1 · **Attack** 5 · **Defense** 8 · **Health Levels** 3 · **Movement Rate** 14m

**Antlers**: 4, Melee

**Notable Skills**: Athletics 10

**Traits**: **Faster Than the Joke Suggests** - Advantage on any roll to avoid being Surprised.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Pope Lick Monster

Lives on and under a rail trestle, and the voice it throws is the only reason anyone gets close enough to fall.

**Soak** 2 · **Attack** 5 · **Defense** 6 · **Health Levels** 4 · **Movement Rate** 10m

**Claws / Hooves**: 4, Melee

**Notable Skills**: Stealth 8

**Traits**: **Voice Mimicry** - imitates a familiar voice or sound from a distance; a Perception roll made to pinpoint its actual location takes Disadvantage as long as it keeps calling. **High Ground** - Advantage on an attack made from an elevated perch against a target below.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Beast of Bladenboro

North Carolina livestock turn up dead with barely a mark on them and no tracks worth following.

**Soak** 3 · **Attack** 6 · **Defense** 5 · **Health Levels** 6 · **Movement Rate** 10m

**Bite**: 5, Melee

**Notable Skills**: Stealth 9

**Traits**: **Silent Kill** - Advantage on an attack made against a target that hasn't detected it.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Ozark Howler

The howl reaches farther than the animal ever does, which is exactly why nobody trusts it isn't close.

**Soak** 2 · **Attack** 5 · **Defense** 6 · **Health Levels** 5 · **Movement Rate** 11m

**Bite / Claws**: 4, Melee

**Notable Skills**: Intimidation 8, Stealth 7

**Traits**: **Howl** (once per encounter) - anyone within Close range takes Disadvantage on their next roll, rattled by a sound with nothing visibly behind it.

</td></tr></table>

---

## Nightmare Creatures

Urban-fantasy and horror archetypes, built entirely on this system's own mechanics. None of these reference Gifts or Ki - every ability below is self-contained, the same way a rattlesnake's venom or a skunk's spray is.

**Nightmare Creatures**: Vampire · Werewolf · Ghoul · Bound Corpse · Lesser Devil · Changeling · Hexcrafter · Skinshifter · Gravefeeder Swarm · Bound Effigy · Wailer · Poltergeist · Hellhound · Mimic · Plague-Bearer

---

<table class="adversary-card"><tr><td markdown="1">

### Vampire

Passes for human right up until it doesn't need to anymore.

**Soak** 2 · **Attack** 7 · **Defense** 7 · **Health Levels** 6 · **Movement Rate** 10m

**Bite / Claws**: 4, Melee. **Blood Drain**: against a target it has Grappled, a successful bite heals it 1 Health Level at the start of each of its turns for the rest of the Scene.

**Notable Skills**: Persuasion 9, Stealth 8

**Traits**: **Predatory Charm** - Advantage on Persuasion and Deception rolls made to get close to a target who hasn't been warned about it. **Sunlight-Averse** - Disadvantage on all rolls made in direct daylight.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Werewolf

Whatever it was before doesn't matter once the shape has finished changing.

**Soak** 3 · **Attack** 7 · **Defense** 6 · **Health Levels** 7 · **Movement Rate** 12m

**Claws / Bite**: 6, Melee

**Notable Skills**: Athletics 9, Perception 8

**Traits**: **Regeneration** - heals 1 Health Level at the start of each of its turns. **Silver Vulnerable** - damage from a silver weapon against it is unsoakable.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Ghoul

Lives off what the cemetery and the morgue don't miss, and what its bite leaves behind doesn't heal clean.

**Soak** 2 · **Attack** 5 · **Defense** 5 · **Health Levels** 5 · **Movement Rate** 8m

**Claws / Bite**: 4, Melee. **Fouled Wound**: a target hit and left untreated takes Disadvantage on Physical rolls until the wound is properly cleaned.

**Notable Skills**: Stealth 8

**Traits**: **Carrion Sense** - Advantage on Perception rolls to locate fresh graves, corpses, or already-wounded targets.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Bound Corpse

Something is still using the body. The body itself stopped being the important part a while ago.

**Soak** 3 · **Attack** 6 · **Defense** 4 · **Health Levels** 6 · **Movement Rate** 7m

**Whatever It's Carrying**: 4, Melee

**Traits**: **Tethered, Not Alive** - immune to Fear and Intimidation, doesn't need to breathe, and taking it past 0 Health Levels only disperses it for the Scene; it re-forms at the corpse's resting place unless the remains themselves are destroyed or the tether is broken by other means (GM's call). **Cold Presence** - anyone within Close range takes Disadvantage on Perception rolls.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Lesser Devil

Doesn't need to win the fight if it can talk someone else into losing it for them.

**Soak** 2 · **Attack** 6 · **Defense** 7 · **Health Levels** 4 · **Movement Rate** 10m (short flight)

**Claws / Horns**: 4, Melee

**Notable Skills**: Persuasion 9, Deception 8

**Traits**: **Goading** - Advantage on Persuasion rolls made to provoke a target into a rash or reckless action. **Fire Vulnerable** - fire damage against it is unsoakable.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Changeling

Wears a borrowed face well enough that the real question isn't whether it's lying, it's how long it's been standing in someone else's life.

**Soak** 1 · **Attack** 3 · **Defense** 8 · **Health Levels** 3 · **Movement Rate** 9m

**Knife** (only if cornered): 2, Melee

**Notable Skills**: Deception 10, Persuasion 9

**Traits**: **Glamour** - Advantage on any Deception roll made to pass as a specific person it has studied closely. **Cracks Both Ways** - a target who succeeds on a Perception roll to see through the glamour gains Advantage on their next roll against it.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Hexcrafter

Ordinary until the ritual finishes. The trick is stopping it before that happens.

**Soak** 0 · **Attack** 3 · **Defense** 6 · **Health Levels** 3 · **Movement Rate** 8m

**Athame** (only if cornered): 1, Melee. **Hex** (once per Scene, requires several uninterrupted rounds to complete - GM's call on timing): a single target within Sight range takes Disadvantage on all rolls for the rest of the Scene once the ritual completes.

**Notable Skills**: Perception 7

**Traits**: **Fragile Without Preparation** - has no combat trait beyond the above; entirely dependent on allies, terrain, or fleeing if interrupted mid-ritual.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Skinshifter

A full night of watching is all it needs before it can wear someone else's face and voice convincingly.

**Soak** 1 · **Attack** 5 · **Defense** 7 · **Health Levels** 4 · **Movement Rate** 9m

**Claws**: 3, Melee

**Notable Skills**: Deception 10, Stealth 8

**Traits**: **Perfect Copy** - after studying a specific target overnight, can take on their appearance and voice; noticing anything wrong requires a Perception roll at Disadvantage. **Reverts Under Stress** - snaps back to its true, indistinct form the instant it takes any Health Level damage.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Gravefeeder Swarm

Rats and roaches don't usually hunt in formation. This mass does.

**Soak** 0 · **Attack** 5 · **Defense** 8 · **Health Levels** 4 · **Movement Rate** 7m

**Bites / Stings**: 3, Melee

**Traits**: **Swarm** (as Rat Swarm). **Unnatural Hunger** - actively pursues rather than scatters, and anything else drawn to a fresh kill gets Advantage on Perception rolls to track a target it has already fed on this Scene.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Bound Effigy

Stone, clay, or scrap, animated and given exactly one job.

**Soak** 6 · **Attack** 6 · **Defense** 3 · **Health Levels** 8 · **Movement Rate** 6m

**Slam**: 6, Melee

**Traits**: **Unliving** - immune to Fear, poison, and disease. **One Weak Point** - a Called Shot against its anchor (a rune, seam, or core, set by the GM on introduction) bypasses its Soak entirely.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Wailer

Almost never lays a hand on anyone. It doesn't need to.

**Soak** 1 · **Attack** 2 · **Defense** 6 · **Health Levels** 3 · **Movement Rate** 8m (drifts)

**Cry** (once per Scene, Mental attack, dice pool 6, Far range, no to-hit roll required - triggered by hearing it): dice resolve per-die against Psyche as normal, a connecting die costing a Sanity Level same as any other Mental attack, and can push a target to Overwhelmed on repeated failures in the same Scene.

**Notable Skills**: Perception 9

**Traits**: **Rarely Physical** - almost never closes to melee; the cry is the entire threat.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Poltergeist

Bound to one room, one object, one unfinished business - and everything not nailed down in that space is a weapon.

**Soak** 1 · **Attack** 5 · **Defense** 8 · **Health Levels** 4 · **Movement Rate** 0m (cannot leave the bound location)

**Thrown Objects**: 4, Close to Far

**Traits**: **Bound to a Place** - immune to anything that doesn't target its anchor object directly (set by the GM on introduction); attacks against anything else pass through with no effect. **Escalates** - its Attack rises by 1, to a maximum of 8, for every full round that passes without anyone leaving the bound location.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Hellhound

Runs a scent down for miles and doesn't lose interest once it's found what it was chasing.

**Soak** 3 · **Attack** 7 · **Defense** 6 · **Health Levels** 6 · **Movement Rate** 14m

**Bite**: 6, Melee. 1 point of damage from a successful bite is unsoakable (the wound smolders).

**Notable Skills**: Perception 9, Athletics 9

**Traits**: **Pack Tactics** (as Feral Dog). **Hunts by Scent** - once it has a target's scent, Advantage on Perception rolls to track them across any terrain short of open water.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Mimic

Furniture, cargo, a parked car, anything still enough to be overlooked - until someone reaches for it.

**Soak** 2 · **Attack** 6 · **Defense** 7 · **Health Levels** 5 · **Movement Rate** 3m (until it strikes)

**Bite / Engulf**: 5, Melee. Advantage on its first attack against a target that approached believing it was ordinary terrain or an object.

**Notable Skills**: Stealth 10

**Traits**: **Grapple on Contact** - a target hit by its first attack is automatically Grappled; breaking free uses the standard contested Grapple rule.

</td></tr></table>

<table class="adversary-card"><tr><td markdown="1">

### Plague-Bearer

Shows up wherever something is already going wrong and makes sure it keeps going wrong.

**Soak** 1 · **Attack** 4 · **Defense** 6 · **Health Levels** 4 · **Movement Rate** 8m

**Cane / Blade**: 3, Melee. **Contagion**: on a successful hit, the target must resist or take Disadvantage on Physical rolls until treated, worsening rather than healing without care.

**Notable Skills**: Intimidation 7, Perception 8

**Traits**: **Wading Through Sickness** - immune to any disease or poison effect, including its own.

</td></tr></table>
