...
Three resources per round, not [source redacted]'s four — **Bonus Action is merged into Reaction** rather than kept as a separate category:
...
- **Reaction** — everyone gets exactly one per round. **Not declared by default** — used opportunistically as needed (an interrupt, a triggered response, a supplementary action off a Skill/Perk/Gift), the same way [source redacted]'s reaction works, unless a specific Perk or Gift explicitly says its use must be declared instead.
...
- **Damage**: how damage is determined once a hit lands (fixed weapon value? scales with the attacker's Degree of Success, tying into the running "DoS drives magnitude" theme? a separate damage roll?), and how it's tracked (hit points vs. a wound/health-level track with escalating penalties, à la [source redacted]).
