...
Inspired by [source redacted]'s split-dice-pool multiple actions rule, adapted to our target-number system: a character can attempt more than one Action in a round by accepting an escalating penalty to their target number on each action after the first.
